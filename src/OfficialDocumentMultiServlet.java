...
public class OfficialDocumentMultiServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String module_name = multi.getParameter("module_name");	if(module_name == null) module_name = "";	//게시판으로 보낼때
		...
		try {
			...
			if ("OFD_write".equals(mode)){
				...
				try{
					//공지공문 입력하기
					docDAO.inputODTTable(id,user_id,user_name,user_rank,div_id,div_code,code,div_name,class_no,slogan,title_name,in_date,enforce_date,receive,reference,sending,subject,firm_name,representative,module_name,mail,mail_add,content,upload_path);
					//첨부파일 입력하기
					...
					//분기하기
					getServletContext().getRequestDispatcher("/ods/OfficialDocument_list.jsp?Cpage="+Cpage+"&Tpage="+Tpage).forward(request,response);
			...
			//공지사항 작성즉시 상신하기
				...
				try{
					...
					//상신 분기하기
					//------------------------------
					getServletContext().getRequestDispatcher("/gw/approval/module/OfficialDocument_AppDirect.jsp?id="+id).forward(request,response);
			...
			//공지사항 수정하기
			else if ("OFD_modify".equals(mode)){
				com.anbtech.dms.db.OfficialDocumentDAO docDAO = new com.anbtech.dms.db.OfficialDocumentDAO(con);
				...
				try{
					//공지공문 수정하기
					docDAO.updateODTTable(id,user_id,user_name,user_rank,div_id,div_code,code,div_name,class_no,slogan,title_name,in_date,enforce_date,receive,reference,sending,subject,firm_name,representative,module_name,mail,mail_add,content,upload_path);
					//첨부파일 수정입력하기
			...
			//공지사항중 첨부파일 개별삭제하기
			else if ("OFD_attachD".equals(mode)){
				com.anbtech.dms.db.OfficialDocumentDAO docDAO = new com.anbtech.dms.db.OfficialDocumentDAO(con);
				...
				try{
					...
					//분기하기
					getServletContext().getRequestDispatcher("/ods/OfficialDocument_modify.jsp").forward(request,response);
