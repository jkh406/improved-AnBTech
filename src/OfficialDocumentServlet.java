...
public class OfficialDocumentServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		//모드 및 현재 페이지 파리미터
		String mode = Hanguel.toHanguel(request.getParameter("mode"))==null?"OFD_L":Hanguel.toHanguel(request.getParameter("mode"));
		...
		try {
			...
			//공지공문 개별내용 보기 (승인전)
				...
				getServletContext().getRequestDispatcher("/ods/OfficialDocument_review.jsp").forward(request,response);
			}
			//공지공문 개별내용 보기 (승인후)
			else if("OFD_A".equals(mode)){	
				//공지공문 내용
				com.anbtech.dms.db.OfficialDocumentDAO docDAO = new com.anbtech.dms.db.OfficialDocumentDAO(con);
				ArrayList table_list = new ArrayList();
				table_list = docDAO.getDoc_Read(id);
				...
				getServletContext().getRequestDispatcher("/ods/OfficialDocument_Appview.jsp").forward(request,response);
			}
			//공지공문 개별내용 수정하기
			else if("OFD_M".equals(mode)){	
				com.anbtech.dms.db.OfficialDocumentDAO docDAO = new com.anbtech.dms.db.OfficialDocumentDAO(con);
				ArrayList table_list = new ArrayList();
			...
			//공지공문 결재상신 Include하기
				...
				getServletContext().getRequestDispatcher("/ods/OfficialDocument_include.jsp").forward(request,response);
			}
		...
		try {
			...
			//공지공문 결재상신 하기
				...
				try{
					...
					masterDAO.setTable(app_mode,doc_pid,doc_subj,writer_id,writer_name,writer_date,doc_line,doc_peri,doc_secu,"","공지공문",id,"ODT");
					
					//전자결재 TABEL (app_master)에 첨부파일 수량 update하기
					masterDAO.setAttacheCount("app_master",doc_pid,atcnt);

					//관련문서 Table에 전자결재 현상태 알려 주기
					masterDAO.setTableLink("OfficialDocument","flag","id",id,"EE");
					con.commit(); // commit한다.
					con.setAutoCommit(true);
