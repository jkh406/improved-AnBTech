...
public class PsmBudgetServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String budget_type = Hanguel.toHanguel(request.getParameter("budget_type"))==null?"1":Hanguel.toHanguel(request.getParameter("budget_type"));
		...
		try {
			...
			if ("bud_prewrite".equals(mode)){
				...
				budgetT = psmDAO.readPsmBudget(pid,psm_code);
				request.setAttribute("BUDGET_table", budgetT);
				...
				status_list = psmDAO.getPsmBudgetList(psm_code);			
				request.setAttribute("BUDGET_List", status_list); 

				//PSM MASTER 데이터 읽기 [예산총액을 알기위해서]
				com.anbtech.psm.entity.psmMasterTable masterT = new com.anbtech.psm.entity.psmMasterTable();
				masterT = psmDAO.readPsmMasterCode(psm_code);
				...
				//분기하기
				getServletContext().getRequestDispatcher("/psm/prs/psmBudgetReg.jsp").forward(request,response);
			}
/*			//리스트 정보
			else if ("bud_list".equals(mode)){
				com.anbtech.psm.db.psmModifyDAO psmDAO = new com.anbtech.psm.db.psmModifyDAO(con);
				...
				//진행 리스트 읽기
				ArrayList status_list = new ArrayList();
				status_list = psmDAO.getPsmBudgetList(psm_code,page,max_display_cnt);			
				request.setAttribute("BUDGET_List", status_list); 

				//페이지로 바로가기 List
				com.anbtech.psm.entity.psmBudgetTable pageL = new com.anbtech.psm.entity.psmBudgetTable();
				pageL = psmDAO.getBudDisplayPage(psm_code,page,max_display_cnt,max_display_page);
				...
				//분기하기
				getServletContext().getRequestDispatcher("/psm/prs/psmBudgetList.jsp").forward(request,response);
			}
*/
		}catch (Exception e){
			//에러출력 페이지로 분기
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String budget_type = multi.getParameter("budget_type")==null?"1":multi.getParameter("budget_type");
...
*/
			...
			//PSM STATUS 상태변경 등록
			else if("bud_write".equals(mode) || "bud_modify".equals(mode) || "bud_delete".equals(mode)) {
				com.anbtech.psm.business.psmBudgetBO budBO = new com.anbtech.psm.business.psmBudgetBO(con);

				con.setAutoCommit(false);	// 트랜잭션을 시작
				con.setTransactionIsolation(Connection.TRANSACTION_SERIALIZABLE);
				try{
					//등록하기
					if("bud_write".equals(mode)) {
						msg = budBO.inputPsmBudget(pid,psm_code,psm_type,comp_name,comp_category,psm_korea,psm_english,psm_start_date,psm_end_date,psm_pm,psm_mgr,psm_budget,psm_user,plan_sum,plan_labor,plan_material,plan_cost,plan_plant,change_desc,budget_type);
					}
					//수정하기
					else if("bud_modify".equals(mode)) {
						msg = budBO.updatePsmStatus(pid,psm_code,psm_type,comp_name,comp_category,psm_korea,psm_english,psm_start_date,psm_end_date,psm_pm,psm_mgr,psm_budget,psm_user,plan_sum,plan_labor,plan_material,plan_cost,plan_plant,change_desc,budget_type);
					}
					//삭제하기
					else if("bud_delete".equals(mode)) {
						msg = budBO.deletePsmBudget(pid,psm_code,budget_type);
					}
					...
					//out.println("	<script>");
					//out.println("	location.href('PsmBudgetServlet?mode=bud_list"+para+"');");
					//out.println("	</script>");

					out.println("	<script>");
					out.println("	location.href('PsmBudgetServlet?mode=bud_prewrite"+para+"');");
					out.println("	</script>"); 
				}catch(Exception e){
					con.rollback();
					con.setAutoCommit(true);
