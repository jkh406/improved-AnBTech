...
public class ProposalBO{
	...
	public ProposalTable getData(String mode,String tablename,String data_id,String ver_code) throws Exception{
		...
		if(security_level.equals("1")) proposal.setSecurityLevel("1급");
		else if(security_level.equals("2")) proposal.setSecurityLevel("2급");
		else if(security_level.equals("3")) proposal.setSecurityLevel("3급");
		else if(security_level.equals("4")) proposal.setSecurityLevel("대외비");
		else if(security_level.equals("5")) proposal.setSecurityLevel("일반문서");

		//문서 보존기간 처리
		String save_period = proposal.getSavePeriod();
		if(save_period.equals("1")) proposal.setSavePeriod("1년");
		else if(save_period.equals("3")) proposal.setSavePeriod("3년");
		else if(save_period.equals("5")) proposal.setSavePeriod("5년");
		else if(save_period.equals("10")) proposal.setSavePeriod("10년");
		else if(save_period.equals("0")) proposal.setSavePeriod("영구");

		//작성언어 처리
		String written_lang = proposal.getWrittenLang();
		if(written_lang.equals("KOR")) proposal.setWrittenLang("한국어");
		else if(written_lang.equals("ENG")) proposal.setWrittenLang("영어");
		else if(written_lang.equals("JPN")) proposal.setWrittenLang("일본어");
		else if(written_lang.equals("CHN")) proposal.setWrittenLang("중국어");
		else if(written_lang.equals("DAU")) proposal.setWrittenLang("독일어");	

		//문서유형 처리
		String doc_type = proposal.getDocType();
		if(doc_type.equals("FILE")) proposal.setDocType("파일형태");
		if(doc_type.equals("BOOK")) proposal.setDocType("책자형태");	
		if(doc_type.equals("SHEET")) proposal.setDocType("SHEET형태");
		if(doc_type.equals("CD")) proposal.setDocType("CD(DVD)형태");	
		if(doc_type.equals("TAPE")) proposal.setDocType("테이프형태");
		if(doc_type.equals("FILM")) proposal.setDocType("필름형태");	
