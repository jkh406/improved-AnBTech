...
public class InDocumentDAO
{
	...
	//	생성자 만들기
	//*******************************************************************/
	public InDocumentDAO(Connection con) 
	{
		this.con = con;
	}
	...
	public int getTotalCount(String login_id,String sItem,String sWord) throws Exception
	{
		...
		query = "SELECT COUNT(*) FROM InDocument_send where ac_id='"+ac_id+"'";	
	...
	public ArrayList getDoc_List (String login_id,String sItem,String sWord,String page,int max_display_cnt) throws Exception
	{
		...
		//query문장 만들기
		query = "SELECT * FROM InDocument_send where ac_id='"+ac_id+"'";	
	...
	public ArrayList getDoc_Read (String id) throws Exception
	{
		...
		//query문장 만들기
		query = "SELECT * FROM InDocument_send where id='"+id+"'";	
		rs = stmt.executeQuery(query);
	...
	* 사내공문 사항 내용 저장하기 
	*******************************************************************/
	public String inputINDTable(String id,String user_id,String user_name,String user_rank,
		...
		String firm_name,String representative,String module_name,String module_add,String mail,String mail_add,
		String content,String upload_path) throws Exception
	{
		...
		String rtn = "";			//리턴값
		//수신부서명을 이용하여 전자우편 대상자를 class_table에서 구하기
		//(수신부서명의 대상자가 없으면 부서명을 리턴하고 끝내기						
		rtn = searchChief(receive);		//정상:사번/이름;,,, 미정상:부서명;...
		if(rtn.indexOf("/") == -1) return rtn;
		else { mail_add = rtn;	rtn = "ok"; }
		...
		Statement stmt = null;
		stmt = con.createStatement();
		String incommon = "INSERT INTO InDocument_send(id,user_id,user_name,user_rank,";
			incommon += "ac_id,ac_code,code,ac_name,class_no,slogan,title_name,in_date,enforce_date,receive,reference,sending,";
			incommon += "subject,bon_path,bon_file,firm_name,representative,module_name,module_add,mail,mail_add) values('";
		...
		String input = incommon+id+"','"+user_id+"','"+user_name+"','"+user_rank+"','";
			...
			input += doc_pat+"','"+id+"','"+firm_name+"','"+representative+"','"+module_name+"','"+module_add+"','";
			input += mail+"','"+mail_add+"')";
	...
	* 사내공문 사항 내용 수정하기 
	*******************************************************************/
	public String updateINDTable(String id,String user_id,String user_name,String user_rank,
		String ac_id,String ac_code,String code,String ac_name,String class_no,String slogan,String title_name,
		String in_date,String enforce_date,String receive,String reference,String sending,String subject,
		String firm_name,String representative,String module_name,String module_add,String mail,String mail_add,
		String content,String upload_path) throws Exception
	{
		...
		Statement stmt = null;
		stmt = con.createStatement();
		String update = "UPDATE InDocument_send set user_id='"+user_id+"',user_name='"+user_name;
			...
			update += "',module_name='"+module_name+"',module_add='"+module_add+"',mail='"+mail+"',mail_add='"+mail_add;
			update += "' where id='"+id+"'";
	...
	private String searchChief (String receive) throws Exception
	{
		...
		for(int i=0; i<div_cnt; i++) for(int j=0; j<3; j++) rec[i][j]="";
		...
		for(int i=0; i<div_cnt; i++) {
			query = "select chief_id from class_table where ac_id = '"+rec[i][0]+"' and isuse='y'";

			rs = stmt.executeQuery(query);
			if(rs.next()) rec[i][2] = rs.getString("chief_id");
		}
		//검사:부서장사번이 있는지 검사 없으면 부서명을 리턴함.
		for(int i=0; i<div_cnt; i++) if(rec[i][2].length()==0) rtn += rec[i][1]+",";
	...
	* 첨부파일 저장한후 Table에 update하기
	 ...
	 private void setAddFileUpdate(String id, String filename, String savename, String filetype, String filesize) throws Exception
	{
		Statement stmt = null;
		stmt = con.createStatement();
		String update = "update InDocument_send set fname='"+filename+"',sname='"+savename+"',ftype='"+filetype+"',fsize='"+filesize+"'";
			update += " where id='"+id+"'";
		int er = stmt.executeUpdate(update);
...
//System.out.println("cnt : del_ext == " + cnt + ":" + del_ext);
		...
		Statement stmt = null;
		stmt = con.createStatement();
		String update = "update InDocument_send set ";
...
//System.out.println("addFile[del_ext][3] " + addFile[del_ext][3] );
	...
	public void deletePid (String id,String upload_path) throws Exception
	{
		...
		//query문장 만들기
		String query  = "SELECT * FROM InDocument_send where id='"+id+"'";
		rs = stmt.executeQuery(query);
		...
		if(dbt || daf) {
			deleteTableLine("InDocument_send",id);		//해당line 삭제하기
		}
		//공통 항목 끝내기 (닫기)
		stmt.close();
		rs.close();
