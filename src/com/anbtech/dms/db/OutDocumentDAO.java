...
public class OutDocumentDAO
{
	...
	//	생성자 만들기
	//*******************************************************************/
	public OutDocumentDAO(Connection con) 
	{
		this.con = con;
	}
	...
	public int getTotalCount(String login_id,String sItem,String sWord) throws Exception
	{
		...
		query = "SELECT COUNT(*) FROM OutDocument_send where user_id='"+login_id+"'";	
	...
	public ArrayList getDoc_List (String login_id,String sItem,String sWord,String page,int max_display_cnt) throws Exception
	{
		...
		//query문장 만들기
		query = "SELECT * FROM OutDocument_send where user_id='"+login_id+"'";	
		query += " and ("+sItem+" like '%"+sWord+"%') order by id desc"; 
		rs = stmt.executeQuery(query);
		...
		while(rs.next() && (show_cnt < max_display_cnt)) { 
				...
				table.setRecCompany(rs.getString("rec_company"));
				table.setRecName(rs.getString("rec_name"));
				table.setRecMail(rs.getString("rec_mail"));
				...
				table.setTel(rs.getString("tel"));	
				table.setFax(rs.getString("fax"));	
				...
				String subLink = "<a href=\"javascript:contentReview('"+pid+"');\">"+subj+"</a>";//승인전
				if(app_id.length() != 0) {
					subLink = "<a href=\"javascript:contentAppview('"+pid+"','"+app_id+"');\">"+subj+"</a>";	 //승인후
				}
				table.setSubject(subLink);							//제목 

				//수정 or 삭제가능 표시
				String subMod="",subDel="";
				if((flg == null) || flg.equals("EN") || flg.equals("EC")) {
					...
					subMod = "<a href=\"javascript:sendEmail('"+pid+"');\"><img src='../ods/images/lt_mail_e.gif' border='0' align='absmiddle'></a>";
				}
	...
	//*******************************************************************
	// DB InDocument_send에서 해당ID QUERY하기 (개별 읽기)
	//*******************************************************************/	
	public ArrayList getDoc_Read (String id) throws Exception
	{
		...
		//query문장 만들기
		query = "SELECT * FROM OutDocument_send where id='"+id+"'";	
		rs = stmt.executeQuery(query);
	...
	* 사외공문 사항 내용 저장하기 
	*******************************************************************/
	public void inputOTDTable(String id,String user_id,String user_name,String user_rank,
		String ac_id,String ac_code,String code,String ac_name,String class_no,String slogan,String title_name,
		String in_date,String enforce_date,String receive,String reference,String sending,
		String rec_company,String rec_name,String rec_mail,String subject,String address,String tel,String fax,
		String firm_name,String representative,String module_name,String content,String upload_path) throws Exception
	{
		//' 없애기
		...
		Statement stmt = null;
		stmt = con.createStatement();
		String incommon = "INSERT INTO OutDocument_send(id,user_id,user_name,user_rank,";
			incommon += "ac_id,ac_code,code,ac_name,class_no,slogan,title_name,in_date,enforce_date,receive,reference,sending,";
			incommon += "rec_company,rec_name,rec_mail,subject,address,tel,fax,bon_path,bon_file,firm_name,representative,module_name) values('";
		
		String input = incommon+id+"','"+user_id+"','"+user_name+"','"+user_rank+"','";
			input += ac_id+"','"+ac_code+"','"+code+"','"+ac_name+"','"+class_no+"','"+slogan+"','"+title_name+"','";
			input += in_date+"','"+enforce_date+"','"+receive+"','"+reference+"','"+sending+"','";
			input += rec_company+"','"+rec_name+"','"+rec_mail+"','"+subject+"','"+address+"','"+tel+"','"+fax+"','";
			input += doc_pat+"','"+id+"','"+firm_name+"','"+representative+"','"+module_name+"')";
	...
	* 사외공문 사항 내용 수정하기 
	*******************************************************************/
	public void updateOTDTable(String id,String user_id,String user_name,String user_rank,
		String ac_id,String ac_code,String code,String ac_name,String class_no,String slogan,String title_name,
		String in_date,String enforce_date,String receive,String reference,String sending,
		String rec_company,String rec_name,String rec_mail,String subject,String address,String tel,String fax,
		String firm_name,String representative,String module_name,String content,String upload_path) throws Exception
	{
		...
		Statement stmt = null;
		stmt = con.createStatement();
		String update = "UPDATE OutDocument_send set user_id='"+user_id+"',user_name='"+user_name;
			...
			update += "',rec_company='"+rec_company+"',rec_name='"+rec_name+"',rec_mail='"+rec_mail;
			update += "',subject='"+subject+"',address='"+address+"',tel='"+tel+"',fax='"+fax;
			update += "',bon_path='"+doc_pat+"',bon_file='"+id+"',firm_name='"+firm_name+"',representative='"+representative;
			update += "',module_name='"+module_name+"' where id='"+id+"'";
	...
	* 첨부파일 저장한후 Table에 update하기
	 ...
	 private void setAddFileUpdate(String id, String filename, String savename, String filetype, String filesize) throws Exception
	{
		Statement stmt = null;
		stmt = con.createStatement();
		String update = "update OutDocument_send set fname='"+filename+"',sname='"+savename+"',ftype='"+filetype+"',fsize='"+filesize+"'";
			update += " where id='"+id+"'";
		int er = stmt.executeUpdate(update);
...
//System.out.println("cnt : del_ext == " + cnt + ":" + del_ext);
		...
		Statement stmt = null;
		stmt = con.createStatement();
		String update = "update OutDocument_send set ";
...
//System.out.println("addFile[del_ext][3] " + addFile[del_ext][3] );
	...
	public void deletePid (String id,String upload_path) throws Exception
	{
		...
		//query문장 만들기
		String query  = "SELECT * FROM OutDocument_send where id='"+id+"'";
		rs = stmt.executeQuery(query);
		...
		if(dbt || daf) {
			deleteTableLine("OutDocument_send",id);		//해당line 삭제하기
		}
		//공통 항목 끝내기 (닫기)
		stmt.close();
		rs.close();
	...
	* 이메일 수신인 update하기
	 *******************************************************************/
	 public void updateReceive(String id,String rec_mail) throws Exception
	{
		Statement stmt = null;
		stmt = con.createStatement();
		String update = "update OutDocument_send set rec_mail='"+rec_mail+"' where id='"+id+"'";
