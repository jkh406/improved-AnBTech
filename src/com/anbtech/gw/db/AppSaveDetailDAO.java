...
public class AppSaveDetailDAO
{
	...
	//	생성자 만들기
	//*******************************************************************/
	public AppSaveDetailDAO(Connection con) 
	{
		this.con = con;
	}

	public AppSaveDetailDAO() 
	{
	...
	public ArrayList getTable_SavePid (String pid) throws Exception
	{
		...
		//query문장 만들기
		query = "SELECT * FROM APP_SAVE where pid='"+pid+"'";	
	...
	private void getReceivers() throws Exception
	{
		...
		if(rs.next()) {
			...
			if(api_cnt > 0) {
				...
				String query = "SELECT receiver,read_date,isopen,receiver_name,receiver_div,receiver_rank FROM APP_RECEIVE where pid='"+this.pid+"'";
	...
	public void setReadInform(String id) throws Exception
	{
		...
		query = "SELECT pid,isopen from APP_RECEIVE where receiver='"+id+"'";
		rs = stmt.executeQuery(query);
