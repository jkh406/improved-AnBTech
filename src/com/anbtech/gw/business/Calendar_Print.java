...
public class Calendar_Print
{
	...
	private String[] indColumns = {"pid","id","sub","myear","mmonth","mday","eyear","emonth","eday","mtime","item","isopen"};
	 ...
	 * 개인 일정 구하기 
	 **************************************************************************/	
	public String Owner_Print(String rid,String ryear,String rmonth) throws SQLException
	{	
	 ...
	 **************************************************************************/	
	public String Other_Print(String rid,String ryear,String rmonth,String type) throws SQLException
	{	
		String Schedule_items = "";
	...
	{	
		...
		while(bean.isAll()) {
			String mon = bean.getData("mmonth"); if(mon.length() == 1) mon = "0"+mon;
			String mda = bean.getData("mday");	 if(mda.length() == 1) mda = "0"+mda;
			String sch_date = bean.getData("myear") + "/" + mon + "/" + mda;
			...
			String SUB = bean.getData("sub");					//제목 
			...
			if((tymd != 0) && (nymd != 0)) {
				...
				//당일일 경우 
				if(diff == 0) {
					cal_dl += sch_date + "@"+SUB+"@" + YN + "@" + sch_item + "@" + sch_time + "@" + PID + "@" + ";";	
				//기간일 경우 
				} else {
					//기간만큼을 출력을 더한다.
					for(int wi = 0; wi <= diff; wi++) {
						...
						if(rYM.equals(chkYM)) {				
							...
							if((wi != 0) && (nYMD.equals(chkymd))) {		//마지막 일자 
								cal_dl += sch_date+"@"+SUB+"@"+YN+"@"+sch_item+"@["+bean.getData("mday")+"]"+sch_time+"@"+PID+"@"+";";
								break;
							} else if((wi == 0) && (!nYMD.equals(chkymd))) { //처음일자 
								cal_dl += sch_date+"@"+SUB+"@"+YN+"@"+sch_item+"@"+sch_time+"@"+PID+"@"+";";
							} else {										//중간 일자 
								cal_dl += sch_date+"@"+SUB+"@"+YN+"@"+sch_item+"@"+bean.getData("mmonth")+"/"+bean.getData("mday")+"~"+bean.getData("emonth")+"/"+bean.getData("eday")+"@"+PID+"@"+";";
							} 
						}//if(요청한 년월만 출력)
					} //for
				} //if 
				...
				if(diff != 0) {
					...
					for(int wi = 0; wi <= diff; wi++) {
						...
						if(!tM.equals(smonth) && (rYM.equals(chkYM))) {	
							sch_date = chkymd.substring(0,4)+"/"+chkymd.substring(4,6)+"/"+chkymd.substring(6,8);
							if((wi != 0) && (nYMD.equals(chkymd))) {		//마지막 일자 
								cal_dl += sch_date+"@"+SUB+"@"+YN+"@"+sch_item+"@["+bean.getData("mmonth")+"/"+bean.getData("mday")+"]"+sch_time+"@"+PID+"@"+";";
								break;
							} else if((wi == 0) && (!nYMD.equals(chkymd))) { //처음일자 
								cal_dl += sch_date+"@"+SUB+"@"+YN+"@"+sch_item+"@"+sch_time+"@"+PID+"@"+";";
							} else {										//중간 일자 
	...
	{	
		...
		while(bean.isAll()) {
			...
			if((tymd != 0) && (nymd != 0)) {
				...
				//당일일 경우 
				if(diff == 0) {
					cal_dl += sch_date+"@"+SUB+"@"+YN+"@"+html_fs+sch_item+"@"+sch_time+"@"+PID+"@"+";";	
				//기간일 경우 
				} else {
					//기간만큼을 출력을 더한다.
					for(int wi = 0; wi <= diff; wi++) {
						...
						if(rYM.equals(chkYM)) {	
							sch_date = chkymd.substring(0,4)+"/"+chkymd.substring(4,6)+"/"+chkymd.substring(6,8);
							if((wi != 0) && (nYMD.equals(chkymd))) {		//마지막 일자 
								cal_dl += sch_date+"@"+SUB+"@"+YN+"@"+html_fs+sch_item+"@"+"["+bean.getData("mday")+"]"+sch_time+"@"+PID+"@"+";";
								break;
							} else if((wi == 0) && (!nYMD.equals(chkymd))) { //처음일자 
								cal_dl += sch_date+"@"+SUB+"@"+YN+"@"+html_fs+sch_item+"@"+sch_time+"@"+PID+"@"+";";
							} else {										//중간 일자 
								cal_dl += sch_date+"@"+SUB+"@"+YN+"@"+html_fs+sch_item+"@"+bean.getData("mmonth")+"/"+bean.getData("mday")+"~"+bean.getData("emonth")+"/"+bean.getData("eday")+"@"+PID+"@"+";";
							} 
						} //if(요청한 년월만 출력)
					} //for
				} //if 
			...
			if((tymd != 0) && (nymd != 0)) {
				...
				if(diff!= 0) {
					...
					for(int wi = 0; wi <= diff; wi++) {
						...
						if(!tM.equals(smonth) && (rYM.equals(chkYM))) {	
							if((wi != 0) && (nYMD.equals(chkymd))) {		//마지막 일자 
								cal_dl += sch_date+"@"+SUB+"@"+YN+"@"+html_fs+sch_item+"@"+"["+bean.getData("mmonth")+"/"+bean.getData("mday")+"]"+sch_time+"@"+PID+"@"+";";
								break;
							} else if((wi == 0) && (!nYMD.equals(chkymd))) { //처음일자 
								cal_dl += sch_date+"@"+SUB+"@"+YN+"@"+html_fs+sch_item+"@"+sch_time+"@"+PID+"@"+";";
							} else {										//중간 일자 
	...
	{	
		...
		int qbyear = qyear - 1;	//1년전
		query = "where id='"+rid+"' and ("+qbyear+" = myear and eyear = "+qyear+" and eyear != 0) and ";
		query += "("+qmonth+" <= emonth) order and isopen='1' by mtime ASC";
