 ...
 *
 * $Id: HashNMap.java,v 1.1 2012/08/03 08:39:17 purepeter Exp $
 *
 * Changes
 * -------
...
public class HashNMap implements Serializable, Cloneable
{
  ...
  /**
   * map에 key/value쌍으로 입력하기(단,입력된값이 같으면 등록되지 않는다)
   *  unique하게 값 입력하기
   *
   * @param key  the key.
   * @param val  the value.
   */
  public void uniqueAdd(final Object key, final Object val)
  {
    final ArrayList v = (ArrayList) table.get(key);
    if (v == null)
    {
	...
	{
		int s = 0;
		for(int i=0; i<v.size(); i++) if(val.toString().equals(v.get(i).toString())) s++;
		if(s == 0) v.add(val);
	}
  }
