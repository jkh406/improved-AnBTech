...
public class pjtNoteDAO
{
	...
	//	생성자 만들기
	//*******************************************************************/
	public pjtNoteDAO(Connection con) 
	{
		this.con = con;
	}
	...
	//*******************************************************************
	//	총 수량 파악하기 [문제점/해당과제 전체수량]
	//*******************************************************************/
	private int getTotalCount(String pjt_code,String note_status,String sItem,String sWord) throws Exception
	{
		//변수 초기화
		Statement stmt = null;
		ResultSet rs = null;
		...
		query = "SELECT COUNT(*) FROM pjt_note where pjt_code='"+pjt_code+"'";
		query += " and note_status like '%"+note_status+"%'";
	...
	// 해당과제의 문제점 List
	//*******************************************************************/	
	public ArrayList getNoteList (String pjt_code,String note_status,String sItem,String sWord,
		String page,int max_display_cnt) throws Exception
	{
		...
		//총갯수 구하기
		total_cnt = getTotalCount(pjt_code,note_status,sItem,sWord);

		//금일일자 구하기 (수정/삭제 허가여부판단)
		String todate = anbdt.getDate();		//yyyy-MM-dd
			
		//query문장 만들기
		query = "SELECT * FROM pjt_note where pjt_code='"+pjt_code+"'";
		query += " and note_status like '%"+note_status+"%'";
		query += " and "+sItem+" like '%"+sWord+"%'"; 
		rs = stmt.executeQuery(query);
		...
		while(rs.next() && (show_cnt < max_display_cnt)) { 
				...
				String note_s = rs.getString("note_status");
				...
				String vnote = "";
				vnote = "<a href=\"javascript:noteSolution('"+pid+"');\">"+note+"</a>";		//미해결시
				if(note_s.equals("1")) {		//상태가 1 이면 해결됨
					vnote = "<a href=\"javascript:noteContent('"+pid+"');\">"+note+"</a>";	//해결시
				}
				table.setNote(vnote);

				//보기,수정,삭제가능 표시 [단,수정/삭제는 당일에 한해 작성자만이 가능함]
				String subMod="",subDel="",subView="";
				if(todate.equals(in_date) && (note_s.equals("0"))) {		//문제점 작성시
					subMod = "<a href=\"javascript:noteModify('"+pid+"');\">수정[N]</a>";
					subDel = "<a href=\"javascript:noteDelete('"+pid+"');\">삭제[N]</a>";
				}else if(todate.equals(sol_date) && (note_s.equals("1"))) {		//문제점 작성시
	...
	// 해당과제 지정된 문제점 상세히 보기
	//*******************************************************************/	
	public ArrayList getNoteRead (String pid) throws Exception
	{
		//변수 초기화
		Statement stmt = null;
		ResultSet rs = null;
		...
		//query문장 만들기
		query = "SELECT * FROM pjt_note where pid='"+pid+"'";	
		rs = stmt.executeQuery(query);
		...
		while(rs.next()) { 
				...
				table.setNoteStatus(rs.getString("note_status"));	
	...
	* 문제점 제시 작성하기 : 신규등록
	*******************************************************************/
	public void inputNote(String pid,String pjt_code,String pjt_name,String node_code,String users,String in_date,
		String book_date,String note,String solution,String note_status) throws Exception
	{
		String input = "";
		Statement stmt = null;
		stmt = con.createStatement();

		input = "INSERT INTO pjt_note(pid,pjt_code,pjt_name,node_code,users,in_date,book_date,note,solution,content,sol_date,";
		input += "note_status) values('";
		input += pid+"','"+pjt_code+"','"+pjt_name+"','"+node_code+"','"+users+"','"+in_date+"','"+book_date+"','";
		input += note+"','"+solution+"','','','"+note_status+"')";
	...
	* 문제점 제시 작성하기 : 수정하기
	*******************************************************************/
	public void updateNote(String pid,String node_code,String book_date,String users,String note,String solution) throws Exception
	{
		String update = "";
		Statement stmt = null;
		stmt = con.createStatement();

		update = "UPDATE pjt_note set node_code='"+node_code+"',book_date='"+book_date;
		update +="',users='"+users+"',note='"+note+"',solution='"+solution;
	...
	* 문제점 해결 작성하기 : 수정하기
	*******************************************************************/
	public void updateContent(String pid,String content,String sol_date,String note_status) throws Exception
	{
		String update = "";
		Statement stmt = null;
		stmt = con.createStatement();

		update = "UPDATE pjt_note set content='"+content+"',sol_date='"+sol_date+"',note_status='"+note_status;
		update += "' where pid='"+pid+"'";
		stmt.executeUpdate(update);
	...
	*******************************************************************/
	public void updateRecovery(String pid) throws Exception
	{
		...
		update = "UPDATE pjt_note set content='',sol_date='',note_status='0' ";
		update += "where pid='"+pid+"'";
		stmt.executeUpdate(update);
	...
	* 문제점 제시 작성하기 : 삭제하기
	*******************************************************************/
	public void deleteNote(String pid) throws Exception
	{
		Statement stmt = null;
		String delete = "";
		stmt = con.createStatement();

		delete = "DELETE from pjt_note where pid='"+pid+"'";
