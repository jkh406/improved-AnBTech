...
public class prsActivityDocDAO
{
	...
	//	생성자 만들기
	//*******************************************************************/
	public prsActivityDocDAO(Connection con) 
	{
		this.con = con;
	}

	public prsActivityDocDAO() 
	{
	...
	public int getAllTotalCount(String sItem,String sWord) throws Exception
	{
		...
		query = "SELECT COUNT(*) FROM prs_docname where type='P'";
	...
	public int getDivTotalCount(String div_code,String sItem,String sWord) throws Exception
	{
		...
		query = "SELECT COUNT(*) FROM prs_docname where type='"+div_code+"'";
		query += " and ("+sItem+" like '%"+sWord+"%')"; 
		rs = stmt.executeQuery(query);
		rs.next();
		int cnt = rs.getInt(1);
	...
	//*******************************************************************/	
	public ArrayList getActivityDocAllList (String login_id,String sItem,String sWord,String page,int max_display_cnt) throws Exception
	{
		//변수 초기화
		Statement stmt = null;
		ResultSet rs = null;
		...
		//query문장 만들기
		query = "SELECT a.pid,a.ph_code,c.ph_name,a.step_code,b.step_name,a.doc_code,a.doc_name,a.type ";
		query += "FROM prs_docname a,prs_step b,prs_phase c ";
		query += "where a.type='P' and a.ph_code=c.ph_code and a.step_code=b.step_code and b.ph_code=c.ph_code";	
		query += " and (a."+sItem+" like '%"+sWord+"%') order by a.ph_code,a.step_code,a.doc_code asc"; 
		rs = stmt.executeQuery(query);
		...
		while(rs.next() && (show_cnt < max_display_cnt)) { 
				...
				String doc_code = rs.getString("doc_code");
				table.setDocCode(doc_code);
				table.setDocName(rs.getString("doc_name"));
				table.setType(rs.getString("type"));	
				
				//document코드가 하부구조[process]에서 사용되었는지 검사하기
				String use = useDocumentAtProcess(doc_code,"P");
	...
	//*******************************************************************/	
	public ArrayList getActivityDocDivList (String login_id,String sItem,String sWord,String page,int max_display_cnt) throws Exception
	{
		//변수 초기화
		Statement stmt = null;
		ResultSet rs = null;
		...
		while(rs.next() && (show_cnt < max_display_cnt)) { 
				...
				//document코드가 하부구조[process]에서 사용되었는지 검사하기
				String use = useDocumentAtProcess(doc_code,login_division);
	...
	//*******************************************************************
	//	산출물이 산출물 표준 프로세스[prs_document]에 사용했는지 판단하기
	//*******************************************************************/	
	public String useDocumentAtProcess (String doc_code,String type) throws Exception
	{
		...
		//부서 관리코드 알아보기
		query  = "SELECT COUNT(*) FROM prs_document ";
		query += "where child_node = '"+doc_code+"' and type = '"+type+"'";
		rs = stmt.executeQuery(query);
		rs.next();
		int cnt = rs.getInt(1);
		if(cnt > 0) rtn = "Y";
	...
	//*******************************************************************
	// 해당docivity QUERY하기 (개별 읽기)
	//*******************************************************************/	
	public ArrayList getActivityDocRead (String pid) throws Exception
	{
		//변수 초기화
		Statement stmt = null;
		ResultSet rs = null;
	...
	* PHASE 내용 저장하기 
	*******************************************************************/
	public void inputActivityDoc(String pid,String ph_code,String step_code,String doc_code,String doc_name,String type) throws Exception
	{
		Statement stmt = null;
		stmt = con.createStatement();
		String input = "INSERT INTO prs_docname(pid,ph_code,step_code,doc_code,doc_name,type) values('";
			input += pid+"','"+ph_code+"','"+step_code+"','"+doc_code+"','"+doc_name+"','"+type+"')";
	...
	* PHASE 내용 수정하기 
	*******************************************************************/
	public void updateActivityDoc(String pid,String ph_code,String step_code,String doc_code,String doc_name,String type) throws Exception
	{

		Statement stmt = null;
		stmt = con.createStatement();
		String update = "UPDATE prs_docname set ph_code='"+ph_code+"',step_code='"+step_code+"',doc_code='"+doc_code;
			update += "',doc_name='"+doc_name+"', type='"+type+"' where pid='"+pid+"'";
	...
	* PHASE 내용 삭제하기 
	*******************************************************************/
	public void deleteActivityDoc(String pid) throws Exception
	{

		Statement stmt = null;
		stmt = con.createStatement();
		String delete = "DELETE from prs_docname where pid='"+pid+"'";
	...
	// type : A:전사, D:부서 
	//*******************************************************************/	
	public String getActivityDocCode (String login_id,String div_code,String step_code,String tag) throws Exception
	{
		//변수 초기화
		String rtn = "";			//리턴데이터
		Statement stmt = null;
		...
		stmt = con.createStatement();
		if(tag.equals("A") && prs_mgr.equals("Y"))
			query = "SELECT doc_code FROM prs_docname where type='P' and step_code='"+step_code+"' order by doc_code desc";		
		else if(tag.equals("D") && prs_mgr.equals("Y"))
			query = "SELECT doc_code FROM prs_docname where type='"+div_code+"' and step_code='"+step_code+"' order by doc_code desc";	
		else return rtn;

		//데이터 담기
		rs = stmt.executeQuery(query);
		if(rs.next()) rtn = rs.getString("doc_code");
	...
	// 변경사유 : 중간삭제,추가
	//*******************************************************************/	
	public void updateActivityDocCode (String login_id,String type,String step_code,String tag) throws Exception
	{
		//변수 초기화
		Statement stmt = null;
		ResultSet rs = null;
		...
		int cnt = 0;
		if(tag.equals("A") && prs_mgr.equals("Y")) {
			cnt = getAllTotalCount("doc_code","");
			if(cnt == 0) return;
		}
		else if(tag.equals("D") && prs_mgr.equals("Y")) {
			cnt = getDivTotalCount(type,"doc_code","");
			if(cnt == 0) return;
		}
		...
		stmt = con.createStatement();
		if(tag.equals("A") && prs_mgr.equals("Y"))
			query = "SELECT * FROM prs_docname where type='P' and step_code='"+step_code+"' order by ph_code,step_code,doc_code asc";		
		else if(tag.equals("D") && prs_mgr.equals("Y"))
			query = "SELECT * FROM prs_docname where type='"+type+"' and step_code='"+step_code+"' order by ph_code,step_code,doc_code asc";	
		else return;
		...
		while(rs.next()) {
			...
			data[n][1] = rs.getString("doc_code");
			n++;
		}
		...
		String no = "",update="",NH="";
		for(int i=0,j=51; i<n; i++,j++) {
			if(data[i][1].length() == 5) NH = data[i][1].substring(0,3);	//예,N0150..
			else NH = data[i][1].substring(0,4);		//step code의 마지막에 알파벳이 있는경우 예,N01A50..
			no = NH+nft.toDigits(j);
			update = "update prs_docname set doc_code='"+no+"' where pid='"+data[i][0]+"'";
