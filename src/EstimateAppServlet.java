...
public class EstimateAppServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		//견적서 정보
		String estimate_no = Hanguel.toHanguel(request.getParameter("estimate_no")); if(estimate_no == null) estimate_no = "";		//견적서번호
		String ver = Hanguel.toHanguel(request.getParameter("ver"));			if(ver == null) ver = "";			//견적버젼
		...
		try {
			...
			com.anbtech.gw.db.AppInputMasterDAO masterDAO = new com.anbtech.gw.db.AppInputMasterDAO(con);	//상신
			com.anbtech.em.business.EstimateBO estBO = new com.anbtech.em.business.EstimateBO(con);			//견적
			con.setAutoCommit(false);	// 트랜잭션을 시작  
			con.setTransactionIsolation(Connection.TRANSACTION_SERIALIZABLE);
			try{
				//전자결재 TABEL (app_master) 입력하기
				masterDAO.setTable(app_mode,doc_pid,doc_subj,writer_id,writer_name,writer_date,doc_line,doc_peri,doc_secu,"","견적서",est_pid,"EST");
				
				//관련문서 Table에 전자결재 상신상태 알려 주기
