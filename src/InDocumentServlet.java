...
public class InDocumentServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			//사내공문 개별내용 보기 (승인전)
				...
				getServletContext().getRequestDispatcher("/ods/InDocument_review.jsp").forward(request,response);
			}
			//사내공문 개별내용 보기 (승인후)
			else if("IND_A".equals(mode)){	
				//사내공문 내용
				com.anbtech.dms.db.InDocumentDAO docDAO = new com.anbtech.dms.db.InDocumentDAO(con);
				ArrayList table_list = new ArrayList();
				...
				getServletContext().getRequestDispatcher("/ods/InDocument_Appview.jsp").forward(request,response);
			}
			//사내공문 개별내용 수정하기
			else if("IND_M".equals(mode)){	
				com.anbtech.dms.db.InDocumentDAO docDAO = new com.anbtech.dms.db.InDocumentDAO(con);
				ArrayList table_list = new ArrayList();
			...
			//사외공문 결재상신 Include하기
				...
				getServletContext().getRequestDispatcher("/ods/InDocument_include.jsp").forward(request,response);
			}
		...
		try {
			...
			//사내공문 결재상신 하기
				...
				try{
					...
					masterDAO.setTable(app_mode,doc_pid,doc_subj,writer_id,writer_name,writer_date,doc_line,doc_peri,doc_secu,"","사내공문",id,"IDS");
					
					//전자결재 TABEL (app_master)에 첨부파일 수량 update하기
					masterDAO.setAttacheCount("app_master",doc_pid,atcnt);

					//관련문서 Table에 전자결재 현상태 알려 주기
					masterDAO.setTableLink("InDocument_send","flag","id",id,"EE");
					con.commit(); // commit한다.
					con.setAutoCommit(true);
