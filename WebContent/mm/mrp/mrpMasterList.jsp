...
<%
	...
	com.anbtech.mm.entity.mrpMasterTable table;
	...
	pageL = (mrpMasterTable)request.getAttribute("PAGE_List");
	...
	table_list = (ArrayList)request.getAttribute("MRP_List");
	table = new mrpMasterTable();
	Iterator table_iter = table_list.iterator();
	
%>

<HTML><HEAD><TITLE>MRP 등록현황</TITLE>
	...
	<TR height=27>
		<TD vAlign=top><!-- 타이틀 및 페이지 정보 -->
			...
			<TBODY>
				...
				<TR bgcolor="#BAC2CD">
					<TD valign='middle' class="title"><img src="../mm/images/blet.gif" align="absmiddle"> <%=factory_name%> MRP 등록현황</TD>
			...
			<TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
				<TBODY>
					...
					<TR vAlign=middle height=23>
						<TD noWrap width=100 align=middle class='list_title'>MRP관리번호</TD>
						...
						<TD noWrap width=6 class='list_title'><IMG src="../mm/images/list_tep2.gif"></TD>
						<TD noWrap width=80 align=middle class='list_title'>BOM기준일</TD>
						<TD noWrap width=6 class='list_title'><IMG src="../mm/images/list_tep2.gif"></TD>
...
<%
	while(table_iter.hasNext()){
		table = (mrpMasterTable)table_iter.next();
		String status = "";
		if(table.getMrpStatus().equals("0")) status = "결재반려";
		else if(table.getMrpStatus().equals("1")) status = "MRP작성";
		else if(table.getMrpStatus().equals("2")) status = "편집확정";
		else if(table.getMrpStatus().equals("3")) status = "MRP확정";
		else if(table.getMrpStatus().equals("4")) status = "구매발주";
%>
					<TR height=23 onmouseover="this.style.backgroundColor='#F5F5F5'" onmouseout="this.style.backgroundColor=''" bgColor=#ffffff>
						<TD align=middle class='list_bg'><%=table.getMrpNo()%></td>
						...
						<TD><IMG height=1 width=1></TD>
						<TD align=middle class='list_bg'><%=table.getMrpStartDate()%></td>
						<TD><IMG height=1 width=1></TD>
						<TD align=middle class='list_bg'><%=table.getPCount()%></td>
						<TD><IMG height=1 width=1></TD>
					</TR>
...
function goSearch()
{
	...
	document.sForm.mode.value='mrp_list';
	document.sForm.page.value='1';
	document.onmousedown=dbclick;
	document.sForm.submit();
}
//MRP편집
function mrpView(pid)
{
	document.sForm.action='../servlet/mrpInfoServlet';
	document.sForm.mode.value='mrp_preview';
