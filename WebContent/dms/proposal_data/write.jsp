...
<%@ page import="java.util.*,com.anbtech.dms.entity.*"%>
...
//	String version = master.getLastVersion();
	...
	/******************************
	 * proposal_data 테이블내용 가져오기
	 ******************************/
	proposal = new ProposalTable();
	proposal = (ProposalTable)request.getAttribute("proposal_data");
	String where_from		= proposal.getWhereFrom()==null?"자체제안":proposal.getWhereFrom();
	String security_level	= proposal.getSecurityLevel()==null?"3":proposal.getSecurityLevel();
	String save_period		= proposal.getSavePeriod()==null?"3":proposal.getSavePeriod();
	String written_lang		= proposal.getWrittenLang()==null?"KOR":proposal.getWrittenLang();
	String doc_type			= proposal.getDocType()==null?"FILE":proposal.getDocType();
	...
	String category_id		= proposal.getCategoryId()==null?"":proposal.getCategoryId();
	String save_url			= proposal.getSaveUrl()==null?"NA":proposal.getSaveUrl();
	String copy_num			= proposal.getCopyNum()==null?"NA":proposal.getCopyNum();
	String writer_s			= proposal.getWriterS()==null?"":proposal.getWriterS();
	String version			= proposal.getVerCode() == null?"1.0":proposal.getVerCode();
...
<%
	...
	while(file_iter.hasNext()){
		file = (ProposalTable)file_iter.next();
...
<form method=post name="writeForm" action='../servlet/AnBDMS?tablename=proposal_data&upload_size=50' enctype='multipart/form-data' style="margin:0">
	...
	<table cellspacing=0 cellpadding=2 width="100%" border=0>
	   <tbody>
		 ...
		 <tr>
           ...
           <td width="37%" height="25" class="bg_04"><%=reg_type%><input type="hidden" name="mode" value="<%=mode%>"></td>
		   ...
		   <%
				...
				if(category.indexOf("10201") == 0) out.print("<input type='hidden' name='where_from' value='자체제안'>자체제안");
				else if(category.indexOf("10202") == 0) out.print("<input type='hidden' name='where_from' value='RFP제안'>RFP제안");
		   %>
		   </td></tr>
         <tr bgcolor=c7c7c7><td height=1 colspan="4"></td></tr>
		 <tr>
	...
	<table cellspacing=0 cellpadding=2 width="100%" border=0>
	   <tbody>
		 ...
		 <tr>
           ...
           <td width="87%" height="25" colspan="3" class="bg_04"><input type=text name="goods_name" size=30 value="<%=goods_name%>" maxlength="50" class="text_01"></td></tr>
		 <tr bgcolor="C7C7C7"><td height="1" colspan="4"></td></tr>
		 <tr>
           <td width="13%" height="25" class="bg_03" background="../dms/images/bg-01.gif">제안사유</td>
           <td width="87%" height="25" colspan="3" class="bg_04"><input type=text name="why_propose" size=30 value="<%=why_propose%>" maxlength="50" class="text_01"></td></tr>
		 <tr bgcolor=c7c7c7><td height=1 colspan="4"></td></tr>
		 <tr>
           <td width="13%" height="25" class="bg_03" background="../dms/images/bg-01.gif">제안회사명</td>
           <td width="37%" height="25" class="bg_04"><input type=text name=company_name size=15 value="<%=comp_name%>" maxlength="25" class="text_01"></td>
           <td width="13%" height="25" class="bg_03" background="../dms/images/bg-01.gif">제안국가명</td>
           <td width="37%" height="25" class="bg_04"><input type=text name=country_name size=15 value="<%=country_name%>" maxlength="25" class="text_01"></td></tr>
         <tr bgcolor=c7c7c7><td height=1 colspan="4"></td></tr>
		 <tr>
           <td width="13%" height="25" class="bg_03" background="../dms/images/bg-01.gif">담당자명</td>
           <td width="37%" height="25" class="bg_04"><input type=text name=customer_name size=15 value="<%=customer_name%>" maxlength="25" class="text_01"></td>
           <td width="13%" height="25" class="bg_03" background="../dms/images/bg-01.gif">담당자전화</td>
           <td width="37%" height="25" class="bg_04"><input type=text name=customer_tel size=15 value="<%=customer_tel%>" maxlength="25" class="text_01"></td></tr>
         <tr bgcolor=c7c7c7><td height=1 colspan="4"></td></tr>
         <tr><td height=10 colspan="4"></td></tr></tbody></table>
	...
	<table cellspacing=0 cellpadding=2 width="100%" border=0>
	   <tbody>
		 <tr>
           ...
           <td width="87%" height="25" colspan="3" class="bg_04"><textarea rows="5" name="why_revision" cols="93"><%=why_revision%></textarea></td></tr>
...
<div id="d_modify_info" class="collapsed">
	...
	<table cellspacing=0 cellpadding=2 width="100%" border=0>
	   <tbody>
		 ...
		 <tr bgcolor="C7C7C7"><td height="1" colspan="4"></td></tr>
         <tr><td height=10 colspan="4"></td></tr></tbody></table>
  <input type="hidden" name="tablename" value="proposal_data"> 
...
<%
	ProposalTable ref = new ProposalTable();
	...
	while(ref_iter.hasNext()){
		ref = (ProposalTable)ref_iter.next();
...
<% if(enable_model.equals("y")){ %>
	if(f.model_code.value == '10'){
		alert("관련모델명을 찾아서 선택하십시오.");
		return;
	}
<%	}	%>

	if(f.subject.value.length < 5){
		alert("문서제목을 입력하십시오.(5자 이상)");
		f.subject.focus();
		return;
	}
	...
	if(f.writer.value == ''){
		alert("담당자를 찾아서 선택하십시오.");
		return;
	}

	if(f.goods_name.value.length < 3){
		alert("제안할 제품명을 입력하십시오.");
		f.goods_name.focus();
		return;
	}

	if(f.why_propose.value.length < 5){
		alert("제안사유를 입력하십시오.(5자 이상)");
		f.why_propose.focus();
		return;
	}

	if(f.country_name.value.length < 1){
		alert("제안국가명을 입력하십시오.");
...
function node_search()
{
	...
	if(pjt_code.length == 0) { alert('과제이름을 먼저 선택하십시오.'); return; }
