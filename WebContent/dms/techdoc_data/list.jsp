...
<%
	...
	table_list = (ArrayList)request.getAttribute("TechdocData_List");
	table = new TechDocTable();
			  ...
			  <TD align=left width='500'>
				  <form method=get action='AnBDMS' name=srForm onSubmit="if(this.searchword.value.length< 2){alert('검색어는 2자이상 입력하셔야 합니다.');this.searchword.value='';this.searchword.focus();return false;}else{return true;}" style="margin:0">
					  <select name="searchscope">
						  ...
						  <option value='register_s'>등록자</option>
					  ...
					  <% if(!category.equals("101") && !mode.equals("processing")){	//기술문서 분류단계에서는 등록할 수 없도록 함.	%>
	  ...
	  <TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
        <TBODY>
			<TR vAlign=middle height=23>
			  ...
			  <TD noWrap width=6 class='list_title'><IMG src="../dms/images/list_tep2.gif"></TD>
			  <TD noWrap width=40 align=middle class='list_title'>버젼</TD>
			  <TD noWrap width=6 class='list_title'><IMG src="../dms/images/list_tep2.gif"></TD>
			  <TD noWrap width=100 align=middle class='list_title'>등록자</TD>
			  <TD noWrap width=6 class='list_title'><IMG src="../dms/images/list_tep2.gif"></TD>
...
<%
	...
	table = (TechDocTable)table_iter.next();
...
function search_detail() {
	...
	var url = "../dms/techdoc_data/search.jsp?category_info=<%=where_category%>&category=" + category + "&mode=" + mode;
	wopen(url,'search_detail','480','305','scrollbars=no,toolbar=no,status=no,resizable=no');
/*
