...
<%
	...
	*********************************************************************/
	...
	/*********************************************************************
	 	해당과제 개별 비용 집행정보 읽기
	*********************************************************************/
	String pid="",node_code="",node_name="";
...
function costModify()
{
	...
	document.eForm.mode.value='PCO_M';
...
{	
	//환율 및 금액 수정여부 알아보기
	var s_exc = '<%=exchange%>';					//저장된 환율
	var s_cost = '<%=node_cost%>';					//저장된 지출금액
	var c_exc = document.eForm.exchange.value;		//적용할 환율
	var c_cost = document.eForm.node_cost.value;	//수정할 지출금액
	if((s_exc==c_exc) && (s_cost==c_cost))return;	//원래값과 같으면 적용안됨.

	//환율적용 소숫점 자릿수 검사
	var point_cnt = 0;		//소숫점 찾기
	for(i=0; i<c_exc.length; i++) {
		if(c_exc.charAt(i) == '.') point_cnt++;
	}
		...
		<TD height=32><!--버튼-->
			<TABLE width='100%' cellSpacing=0 cellPadding=0 border=0>
				...
				<TR>
					...
					<TD align=left width=''>
						<a href='javascript:costModify()'>
						<img src='../pjt/images/bt_modify.gif' border='0'></a>
					</TD>
				</TR>
				</TBODY>
			</TABLE>
...
<TABLE  cellSpacing=0 cellPadding=0 width="100%" border=0 valign="top">
	...
	<TR height=27><!-- 타이틀 및 페이지 정보 -->
		...
		<TABLE height=27 cellSpacing=0 cellPadding=0 width="100%">
			...
			<TR bgcolor="#BAC2CD">
				<TD valign='middle' class="title"><img src="../pjt/images/blet.gif" align="absmiddle"> 해당계정과목 수정하기</TD>
			</TR>
			</TBODY>
		</TABLE>
		</TD>
	...
	<tr>
		...
		<table cellspacing=0 cellpadding=2 width="100%" border=0>
			   ...
			   <td width="40%" height="25" class="bg_04">
					...
					<%
						...
						for(int i=0; i<man_cnt; i++) {
							if(user_id.equals(member[i][0])) sel = "selected";
							else sel = "";
							out.print("<option "+sel+" value='"+member[i][0]+"|"+member[i][1]+"'>");
							out.println(member[i][1]+" "+member[i][2]+"</option>");
						} 
					%>
					</select></td>
			   ...
			   <td width="40%" height="25" class="bg_04">
					...
					<%
						...
						for(int ti=1; ti<node_cnt; ti++) {
							...
							out.print("<option "+nsel+" value='"+process[ti][0]+"|"+process[ti][1]+"'>");
			   ...
			   <td width="40%" height="25" class="bg_04">
					...
					<%
						...
						for(int ti=0; ti<s_code.length; ti++) {
							if(cost_type.equals(s_code[ti])) tsel = "selected"; 
							else tsel = "";
			...
			<tr>
			   ...
			   <td width="40%" height="25" class="bg_04">
					<input type='text' name='node_cost' value='<%=node_cost%>' size=15 maxlength=16 style="text-align:right;" onKeyPress="isNumObj(this);"> [원]</td>
			   <td width="10%" height="25" class="bg_03" background="../pjt/images/bg-01.gif">환 율</td>
			   <td width="40%" height="25" class="bg_04">
					<input type='text' name='exchange' value='<%=exchange%>' size=15 maxlength=16 style="text-align:right;" OnBlur="javascript:exchangeM();"> [환율입력시 지출금액에 적용됨]</td>
			</tr>
			<tr bgcolor="c7c7c7"><td height=1 colspan="4"></td></tr>
			<tr>
			   <td width="10%" height="25" class="bg_03" background="../pjt/images/bg-01.gif">비 고</td>
			   <td width="40%" height="25" class="bg_04" colspan=3>
					<textarea name='remark' rows='5' cols='80'><%=remark%></textarea></td>
			</tr>
			<tr bgcolor="c7c7c7"><td height=1 colspan="4"></td></tr>
			<tr><td height=10 colspan="4"></td></tr>
			</tbody>
