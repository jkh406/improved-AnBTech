...
<%			
	...
	*********************************************************************/
	...
	query = " where type='P' and ph_code='"+ph_code+"' order by step_code ASC";
	bean.setSearchWrite(query);
	bean.init_write();
	...
	*********************************************************************/
	...
	String[] actColumn = {"ph_code","step_code","doc_code"};
	bean.setTable("prs_docname");		
	bean.setColumns(actColumn);
	query = " where type='P' and ph_code='"+ph_code+"' and step_code='"+step_code+"' order by doc_code DESC";
	bean.setSearchWrite(query);
	bean.init_write();

	String doc_code = step_code+"50";
	if(bean.isAll()) doc_code = bean.getData("doc_code");
	
	//하부구조 구성후 신규로 step code가 알파벳이 추가된 경우 예,N01A N03B ....
	int num = 0;
	if(doc_code.length() == 5) num = Integer.parseInt(doc_code.substring(3,5))+1;
	else num = Integer.parseInt(doc_code.substring(4,6))+1;

	normalFormat fmt = new com.anbtech.util.normalFormat("00");				
	doc_code = step_code+fmt.toDigits(num);
...
function sendSave()
{
	...
	document.eForm.mode.value='DOC_WA';	
	document.eForm.submit();
}
//개발단계명 선택하여 관리항목코드 신규생성
function selectPhCode()
{
	document.eForm.action="docAll_write.jsp";
	...
	<TR>
		<TD height=27><!--타이틀-->
			<TABLE height=27 cellSpacing=0 cellPadding=0 width="100%">
				...
				<TR bgcolor="#BAC2CD">
					<TD valign='middle' class="title"><img src="../images/blet.gif">전사공통 산출물등록</TD>
				</TR>
				</TBODY>
			</TABLE>
		</TD>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr>
		...
		<table cellspacing=0 cellpadding=2 width="100%" border=0>
			...
			<tr bgcolor="c7c7c7"><td height=1 colspan="4"></td></tr>
			<tr>
			   <td width="20%" height="25" class="bg_03" background="../images/bg-01.gif">산출물코드</td>
			   <td width="80%" height="25" class="bg_04"><%=doc_code%>
					<input type='hidden' name='doc_code' value='<%=doc_code%>'></td>
			</tr>
			<tr bgcolor="c7c7c7"><td height=1 colspan="4"></td></tr>
			<tr>
			   <td width="20%" height="25" class="bg_03" background="../images/bg-01.gif">산출물이름</td>
			   <td width="80%" height="25" class="bg_04">
					<input type='text' size=30 name='doc_name' value=''></td>
			</tr>
			<tr bgcolor="C7C7C7"><td height="1" colspan="4"></td></tr>
