...
<%			
	...
	*********************************************************************/
	...
	query = " where type='"+div_code+"' and ph_code='"+ph_code+"' order by step_code DESC";
	bean.setSearchWrite(query);
	bean.init_write();
	...
	*********************************************************************/
	...
	if(tmp_step_code.length() == 3)		//알파벳 확장코드가 없는경우 (예,N04A N04B ...)
		use = stepDAO.useStepAtActivity (step_code,div_code);		//P:전사공통, 부서코드:부서공통

	//다시 step code을구하기
	if(use.equals("Y")) {											//사용됨
		step_code = stepDAO.searchStepCode(ph_code,tmp_step_code,div_code);
	}
...
function sendSave()
{
	...
	document.eForm.mode.value='STP_WD';	
	document.eForm.submit();
}
//개발단계명 선택하여 관리항목코드 신규생성
function selectPhCode()
{
	document.eForm.action="stepDiv_write.jsp";
	document.eForm.submit();
}
-->
</script>
	...
	<TR>
		<TD height=27><!--타이틀-->
			<TABLE height=27 cellSpacing=0 cellPadding=0 width="100%">
				...
				<TR bgcolor="#BAC2CD">
					<TD valign='middle' class="title"><img src="../images/blet.gif">부서공통 관리항목[Step]등록</TD>
				</TR>
				</TBODY>
			</TABLE>
		</TD>
