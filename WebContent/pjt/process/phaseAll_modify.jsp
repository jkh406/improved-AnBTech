...
function sendModify()
{
	...
	var ts = rtn.substring(1,3);	//일련번호 2자리 숫자
	if(rtn.length == 0) { alert('개발단계코드를 지정하십시요.');  return; }
	else if(rtn.length != 3) { alert('3자리만 가능합니다.');  return; }
	else if(!isNaN(hs)) { alert('첫문자는 영문대문자로 입력하십시요.');  return; }
	else if(isNaN(ts)) { alert('영문자뒤의 두자리는 숫자로 입력하십시요.');  return; }
	...
	document.eForm.mode.value='PHA_MA';	
	...
	<TR>
		<TD height=27><!--타이틀-->
			<TABLE height=27 cellSpacing=0 cellPadding=0 width="100%">
				...
				<TR bgcolor="#BAC2CD">
					<TD valign='middle' class="title"><img src="../pjt/images/blet.gif">전사공통 개발단계[Phase]수정</TD>
				</TR>
				</TBODY>
			</TABLE>
		</TD>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr>
		...
		<table cellspacing=0 cellpadding=2 width="100%" border=0>
			...
			<tr>
			   <td width="20%" height="25" class="bg_03" background="../pjt/images/bg-01.gif">개발단계코드</td>
			   <td width="80%" height="25" class="bg_04">
					<input type='text' name='ph_code' value='<%=ph_code%>'> 영문대문자(1)+숫자(2)로 수정합니다.</td>
			</tr>
			<tr bgcolor="c7c7c7"><td height=1 colspan="4"></td></tr>
			<tr>
			   <td width="20%" height="25" class="bg_03" background="../pjt/images/bg-01.gif">개발단계이름</td>
			   <td width="80%" height="25" class="bg_04">
					<input type='text' name='ph_name' value='<%=ph_name%>'></td>
			</tr>
			<tr bgcolor="C7C7C7"><td height="1" colspan="4"></td></tr>
