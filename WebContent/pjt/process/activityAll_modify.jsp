...
<%	
	...
	*********************************************************************/
	...
	if(table_iter.hasNext()){
		...
		act_code=table.getActCode();				//act code
		act_name=table.getActName();				//act name
...
function sendModify()
{
	...
	rtn = document.eForm.act_code.value;
	if(rtn.length == 0) { alert('실행항목코드를 수정하십시요.');  return; }
	rtn = document.eForm.act_name.value;
	if(rtn.length == 0) { alert('실행항목이름을 수정하십시요.');  return; }

	//처리중 메시지 출력
	document.all['lding'].style.visibility="visible";

	document.eForm.action='../servlet/prsCodeServlet';
	document.eForm.mode.value='ACT_MA';	
	...
	<TR>
		<TD height=27><!--타이틀-->
			<TABLE height=27 cellSpacing=0 cellPadding=0 width="100%">
				...
				<TR bgcolor="#BAC2CD">
					<TD valign='middle' class="title"><img src="../pjt/images/blet.gif">전사공통 실행항목[Step]수정</TD>
				</TR>
				</TBODY>
			</TABLE>
		</TD>
		...
		<table cellspacing=0 cellpadding=2 width="100%" border=0>
			...
			<tr>
			   ...
			   <td width="80%" height="25" class="bg_04">
					<input type='text' name='act_name' value='<%=act_name%>'></td>
			</tr>
			<tr bgcolor="C7C7C7"><td height="1" colspan="4"></td></tr>
