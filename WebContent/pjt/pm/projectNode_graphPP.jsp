...
<%@ page		
	info= "진행중과제 Gantt Chart[진행율/계획일]보기"		
...
<%
	...
	while(sch_iter.hasNext()) {
		...
		//예상진행일수(진행율로 계산)
		if(sch.getProgress() != 1.0) {
			String ps = Double.toString(sch.getProgress() * sch.getPlanCnt());	
			...
			<TR>
				...
				<TD align=left width=''>
					...
					<select name='v' onChange='javascript:goView()' style=font-size:9pt;color='black';>
						...
						<option selected value='ganttPP'>그래프[진행율/계획일]</option>
...
<TABLE cellSpacing=0 cellPadding=0 width="100%" border=1 valign='top'>
	<TR vAlign=center bgColor=#E6F5FF height=22>
		...
		<TD noWrap width=100% background="../pjt/img/list_bg.gif">
			...
			//년월 표시하기
			//--------------------------
			int yy = Integer.parseInt(schedule[0][1].substring(0,4)); //계획시작 년도
			int mm = Integer.parseInt(schedule[0][1].substring(4,6)); //계획시작 월
			int dd = Integer.parseInt(schedule[0][1].substring(6,8)); //계획시작 일
			...
			for(int i=0; i<pn_cnt; i++) {
				...
				colspan = anbdt.getDatePeriod(cy,cm,cd);			//남은일수 구하기
