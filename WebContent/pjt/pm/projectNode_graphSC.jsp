...
<%@ page		
	info= "진행중과제 Gantt Chart[실적일/수정일]보기"		
...
<%
	...
	while(sch_iter.hasNext()) {
		...
		schedule[m][5] = sch.getRstStartDate();						//실적시작일
		schedule[m][6] = sch.getRstEndDate();						//실적시작일
			...
			<TR>
				...
				<TD align=left width=''>
					...
					<select name='v' onChange='javascript:goView()' style=font-size:9pt;color='black';>
						...
						<option selected value='ganttSC'>그래프[실적일/수정일]</option>
					</select>
					</form>
				</TD>
			</TR>
	...
	<TR vAlign=center bgColor=#E6F5FF height=22>
		...
		<TD noWrap width=100% background="../pjt/img/list_bg.gif">
			...
			for(int r=1; r<sch_cnt; r++) {
				...
				String con = "0",pjsd="",pjed="",pcsd="",pced="";//출력표기,계획시작/종료일,수정시작,종료일
				String prsd="",pred="",ycon="",flag="";//실적시작일,종료일,실적일이수정일지나가면,실적종료
				for(int i=0; i<pn_cnt; i++) {
					...
					if(schedule[r][5].length() == 8) prsd=anbdt.getSepDate(schedule[r][5],"/");//실적시작일 
					if(schedule[r][6].length() == 8) pred=anbdt.getSepDate(schedule[r][6],"/");//실적종료일 
					...
					if(con.equals("1")) {
						...
						out.print("<div id=z"+r+i+" style=\"position:absolute;background-color:#FEFEED;width:250;	height:50;padding-top:5px ;padding-left:5px ;font:9pt '돋움체';border:#C0C0C0 1px solid;visibility:hidden;\"><font color=#009966>["+schedule[r][0]+"]</font><br>계획기간:"+pjsd+" ~ "+pjed+"<br>수정기간:"+pcsd+" ~ "+pced+"<br>실적기간:"+prsd+" ~ "+pred+"</div>");
						out.println("</TD>");
					} 
				...
				for(int i=0; i<pn_cnt; i++) {
					pdate = str.repWord(anbdt.getDate(py,pm,pd,i),"/",""); 
					//시작일
					if(pdate.equals(schedule[r][5])) con = "1";
					...
					//실적일 조정 [실적종료일이 없으면]
					if(schedule[r][6].length() == 0) con = "0";

					//표시하기 : 실적시작일
					if(con.equals("1")) {
						if(ycon.equals("2")) 		//지연진행
							out.print("<TD noWrap width=5 height=10 align=left bgcolor='red' ");
						else if(ycon.equals("3")) 	//지연완료
							out.print("<TD noWrap width=5 height=10 align=left bgcolor='66cccc' ");
						else 
						...
						out.print("<div id=z0"+r+i+" style=\"position:absolute;background-color:#FEFEED;width:250;	height:50;padding-top:5px ;padding-left:5px ;font:9pt '돋움체';border:#C0C0C0 1px solid;visibility:hidden;\"><font color=#009966>["+schedule[r][0]+"]</font><br>계획기간:"+pjsd+" ~ "+pjed+"<br>수정기간:"+pcsd+" ~ "+pced+"<br>실적기간:"+prsd+" ~ "+pred+"</div>");
						out.println("</TD>");
					} 
					//금일표시
					else if(pdate.equals(anbdt.getDateNoformat())) {		
						out.println("<TD noWrap width=5 height=10 align=right bgcolor='ff9999'></TD>");
					}
					//실적종료일
					else {
						if(ycon.equals("1")) {	//조기완료
							out.print("<TD noWrap width=5 height=10 align=left bgcolor='gray' ");
						} else {				//그외표시없음		
							out.print("<TD noWrap width=5 height=10 align=left bgcolor='white' ");
						}
						out.println("</TD>");
					}

					//실적종료일 : 조기완료와 종료이후 표현
					if(pdate.equals(schedule[r][6])) { con="0"; flag="1"; }
						
					//1.실적 종료일 기준 조기완료 그래프로 표현하기[option사항]
					if(flag.equals("1") && (progress.equals("100.0"))) {
						int cpd = Integer.parseInt(schedule[r][2]); //수정기간 종료일
						int crd = Integer.parseInt(schedule[r][6]); //실적 종료일

						if(cpd == crd) ycon=""; 		//더이상 표현하지 않음
						else if(crd < cpd) ycon="1";	//계획대비일찍끝남
						//else if(crd > cpd) ycon="2";	//계획대비 늦게 끝남,or 진행중
					}
					if(pdate.equals(schedule[r][2])) { flag="0"; ycon=""; }	//flag종료

					//2.수정기간 종료일기준 지연되는 영역 그래프로 표현하기[option사항]
					if(con.equals("1")) {					//실적 그래프 표현
						if(pdate.equals(schedule[r][2])) {	//수정기간 종료일 기준 적색표시
							ycon="2";								//지연 진행
							if(progress.equals("100.0")) ycon="3";	//지연 완료
						} else if(pdate.equals(schedule[r][6])) {	//종료일 기준 표시없음
							ycon="";								//표시없음
						}
					}
