...
//out.println(Scompany + ":" + Cname +":" + Cname_id);
	...
	String arg = "";
	if(arg1.length() != 0) arg += arg1;
	if(arg2.length() != 0) arg += ","+arg2;
	if(arg3.length() != 0) arg += ","+arg3;
...
//	String query = "where a.ap_id=b.company_no and a.at_id=c.at_id and a.au_id=d.id ";	//기본조건
	String[] dbColumns ={"a.*","b.name_kor c_name","c.name_kor p_name","c.writer_info w_name"};
	String query = "where a.ap_id=b.company_no and a.ap_id=c.company_no ";
	...
	query += "order by "+arg;	//정렬
	
	bean.setTable("history_table a,company_customer b,personal_customer c");
	...
	while(bean.isAll()) { 
		...
		if(Sdate.length() != 0) { 
			...
			if(dd != null) {
				...
				cdate = anbdt.getSepDate(dd,"-");					//날자 yyyy-mm-dd
			} else dayi = 0;
			...
			out.println("<TD align=center valign=top>" + str.repWord(bean.getData("s_time"),"/","~") + "</TD>"); 
		}

		if((Scompany.length() != 0) && (Cname_id.equals("all")))		//기업명
			out.println("<TD align=left valign=top>" + bean.getData("c_name") + "</TD>");
		if((Scustomer.length() != 0) && (Sname_id.equals("all")))		//고객명
			out.println("<TD align=center valign=top>" + bean.getData("p_name") + "</TD>");
		if(Spurpose.length() != 0)										//목적
			out.println("<TD align=center valign=top>" + bean.getData("class") + "</TD>");
		if(Subject.length() != 0)										//제목
			out.println("<TD align=left valign=top>" + bean.getData("subject") + "</TD>");
		if(Content.length() != 0) {										//내용
			//본문내용 넣기 (긴내용 중간에 리턴키(br)넣기)
			String bonmun = bean.getData("content");
			Content = "";
		...
		String w_name = bean.getData("w_name");
		w_name = w_name.substring(w_name.lastIndexOf('/')+1,w_name.length());
		out.println("<TD align=center valign=top>"+w_name+"&nbsp;</TD>"); //작성자
		out.println("</TR>");
	} //while 
