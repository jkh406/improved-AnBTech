...
<%
	...
	while(app_iter.hasNext()) {
		...
		if(app.getApStatus().equals("기안")) {
			...
			if((PROCESS.equals("APP_BOX") || PROCESS.equals("DEL_BOX")) && app.getApStatus().equals("통보")){
					//통보자는 결재후 결재선에서 제외하고 의견란으로
			} else if(PROCESS.equals("APP_BOX") || PROCESS.equals("APP_BRP")) {
	...
	// 보고서 정보 알아보기
	*********************************************************************/	
	String[] Column = {"ac_name","user_name","user_rank","in_date","bon_path","bon_file",
						"bg_dest","bg_purpose","fname","sname","ftype","fsize"};
	bean.setTable("bogoseo_master");			
	bean.setColumns(Column);
	bean.setOrder("ac_name ASC");	
	query = "where (bg_id ='"+doc_id+"')";
	...
	while(bean.isAll()) {
		...
		destination = bean.getData("bg_dest");		//출장지
		purpose = bean.getData("bg_purpose");		//출장목적
	...
	//작성년월일 구하기
	String wyear = doc_date.substring(0,4);			//작성년
	String wmonth = doc_date.substring(5,7);		//	  월
	String wdate = doc_date.substring(8,10);		//	  일
	...
	<TR><TD height=27><!--타이틀-->
		<TABLE height=27 cellSpacing=0 cellPadding=0 width="100%">
		  <TBODY>
			...
			<TR bgcolor="#BAC2CD">
			  <TD valign='middle' class="title"><img src="../../images/blet.gif"> 출장보고서</TD></TR></TBODY>
		</TABLE></TD></TR>
			  ...
			  <TD align=left width=500>
					...
					if(PROCESS.equals("APP_ING")) {  %>
						<% if(doc_ste.equals("APV") || doc_ste.equals("APL")) { //검토, 승인단계 %>
							<a href='Javascript:winDecision();'>
								<img src='../../images/bt_commit_app.gif' align='absmiddle' border='0' alt='승인'></a>
							<a href='Javascript:winReject();'>
								<img src='../../images/bt_reject_app.gif' align='absmiddle' border='0' alt='반려'></a>
						<% } else { //협조단계 %>
							<a href='Javascript:winDecision();'>
								<img src='../../images/bt_commit_app.gif' align='absmiddle' border='0' alt='합의'></a>
							<a href='Javascript:winReject();'>
								<img src='../../images/bt_reject_app.gif' align='absmiddle' border='0' alt='반려'></a>
						<% } %>
							<a href='Javascript:winprint(<%=pid%>);'>
								<img src='../../images/bt_print.gif' align='absmiddle' border='0' alt='인쇄'></a>
							<a href='Javascript:history.go(-1);'>
								<img src='../../images/bt_list.gif' align='absmiddle' border='0' alt='목록'></a>
					<% } 
					//재작성및 View 문서	
					else { %>
						<% if(PROCESS.equals("TMP_BOX") || PROCESS.equals("REJ_BOX")) {	//임시보관함,반려함 처리 %> 
							<a href='Javascript:winRewrite();'>
								<img src='../../images/bt_rewrite.gif' align='absmiddle' border='0' alt='재작성'></a>
							<a href='Javascript:winDelete(<%=pid%>);'>
								<img src='../../images/bt_del.gif' align='absmiddle' border='0' alt='삭제'></a>
						<%	} %>
						
						<!-- 기결문서 통보자 메뉴 : 200408 : 관련문서 -->
						<% if(PROCESS.equals("APP_BOX") && wid.equals(sl.id)) {	//기결함,기안자문서 통보자지정 %> 
							<a href='Javascript:addReceive();'>
								<img src='../../images/bt_addRec.gif' align='absmiddle' border='0' alt='통보자지정'></a>
						<%	} %>
						<!-- 기안자의 상신취소[기안자 바로 앞단계시] : 200408 : 공통 -->
						<% if(app_cancel.equals("Y")) {		//상신취소 %> 
							<a href='Javascript:appCancel(<%=pid%>);'>
								<img src='../../images/bt_app_cancel.gif' align='absmiddle' border='0' alt='상신취소'></a>
						<%	} %>
						<!-- 삭제문서로 전달받을경우 와 그렇지 않을경우 메뉴 : 200408 : 공통 -->
						<% if(PROCESS.equals("DEL_BOX")) {	%>
							<a href='Javascript:winclose();'>
							<img src='../../images/bt_close.gif' align='absmiddle' border='0' alt='닫기'></a>
						<% } else { %>
							<a href='Javascript:winprint(<%=pid%>);'>
							<img src='../../images/bt_print.gif' align='absmiddle' border='0' alt='인쇄'></a>
							<a href='Javascript:history.go(-1);'>
	...
	<table cellspacing=0 cellpadding=2 width="100%" border=0>
	   <tbody>
		 ...
		 <tr>
           ...
           <td width="37%" height="25" class="bg_04"><%=destination%></td>
           <td width="13%" height="25" class="bg_03" background="../../images/bg-01.gif">출장목적</td>
           <td width="37%" height="25" class="bg_04"><%=purpose%></td></tr>
         <tr bgcolor=c7c7c7><td height=1 colspan="4"></td></tr>
		 <tr>
           <td width="13%" height="25" class="bg_03" background="../../images/bg-01.gif">출장내용</td>
...
function winRewrite()
{
	...
	document.eForm.action = "chuljang_bogo_FP_Rewrite.jsp";
...
function addReceive()
{
	url = "../searchReceivers.jsp?pid="+'<%=pid%>'; 
	wopen(url,"rec","520","467","scrollbars=no,toolbar=no,status=no,resizable=no");
}
//상신취소 진행하기
