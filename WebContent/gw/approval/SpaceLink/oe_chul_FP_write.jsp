...
/*
	...
	String doc_oechul = "";
	while(bean.isAll()) {
		doc_oechul = bean.getData("ys_name");			//외출관리코드
	} //while
*/
	...
	*********************************************************************/
	...
	String StartTime = multi.getParameter("hdStartTime"); 
			if(StartTime == null) StartTime = hrs+":00";							//시작시간
	String EndTime = multi.getParameter("hdEndTime");
			if(EndTime == null) EndTime = hrs+":30";								//종료시간
%>
...
var code = '<%=doc_oechul%>';
if(code.length == 0) {
	alert("외출코드가 등록되어있지 않습니다. 관리자에게 문의후 사용하십시요.");
}
//동행자 찾기
...
function eleApprovalRequest()
{
	...
	else if(eForm.dest.value == "") { alert("행선지을 입력하십시오."); return;}
	else if(eForm.purpose.value == "") { alert("외출사유를 입력하십시오."); return;}
