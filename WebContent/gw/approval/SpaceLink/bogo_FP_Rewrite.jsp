...
<%
	...
	*********************************************************************/	
	//첨부파일 개별로 읽기 from bogo_FP_view.jsp
	...
	*********************************************************************/
	...
	String prj_name = multi.getParameter("prj_name"); if(prj_name == null) prj_name = "";	//프로젝트명
	String ap_name = multi.getParameter("ap_name"); if(ap_name == null) ap_name = "";	//발주처
	String subject = multi.getParameter("subject"); if(subject == null) subject = "";	//제목
	String content = multi.getParameter("content"); if(content == null) content = "";	//본문
	...
	if(req.equals("ADD_DEL")) {
		...
		String update = "update bogoseo_master set ";
		
		for(int i=0; i<cnt; i++) {
			if(i == del_ext) {	//내용을 제외한 
				fname += " |";
				ftype += " |";
				fsize += " |";
				sname += " |";
		...
		//Tabel update 하기
		update += "fname='"+fname+"',sname='"+sname+"',ftype='"+ftype+"',fsize='"+fsize+"' where bg_id="+old_id;
...
<form action="bogo_FP_Rewrite.jsp" name="eForm" method="post" encType="multipart/form-data" style="margin:0">
	...
	<TR><TD height=27><!--타이틀-->
		<TABLE height=27 cellSpacing=0 cellPadding=0 width="100%">
		  <TBODY>
			...
			<TR bgcolor="#BAC2CD">
			  <TD valign='middle' class="title"><img src="../../images/blet.gif"> 일반보고서</TD></TR></TBODY>
		</TABLE></TD></TR>
  ...
  <tr><td align="center">
	...
	<table cellspacing=0 cellpadding=2 width="100%" border=0>
	   <tbody>
		 ...
		 <tr>
           <td width="13%" height="25" class="bg_03" background="../../images/bg-01.gif">보고일자</td>
         ...
         <tr bgcolor=c7c7c7><td height=1 colspan="4"></td></tr>
		 <tr>
           <td width="13%" height="25" class="bg_03" background="../../images/bg-01.gif">프로젝트명</td>
           <td width="37%" height="25" class="bg_04"><input size=30 type="text" name="prj_name" value='<%=prj_name%>'><a href="Javascript:sel_pjt_code();"><img src="../../images/bt_search.gif" border="0" align="absmiddle"></a></td>
           <td width="13%" height="25" class="bg_03" background="../../images/bg-01.gif">발주처</td>
           <td width="37%" height="25" class="bg_04"><input size=15 type="text" name="ap_name" value='<%=ap_name%>'></td></tr>
         <tr bgcolor=c7c7c7><td height=1 colspan="4"></td></tr>
		 <tr>
           <td width="13%" height="25" class="bg_03" background="../../images/bg-01.gif">제목</td>
           <td width="87%" height="25" colspan="3" class="bg_04"><input size=70 type="text" name="subject" value='<%=subject%>' class='text_01'></td></tr>
		 <tr bgcolor="C7C7C7"><td height="1" colspan="4"></td></tr>
		 <tr>
           ...
           <td width="87%" height="25" colspan="3" class="bg_04">
				 <TABLE><TBODY>
					<TR><TD>
						<a href="javascript:FormResizeInc(document.eForm.content)">
							<img src='../../images/up2.gif' border='0'></a>
						<a href="javascript:FormResizeDec(document.eForm.content)"><img src='../../images/down2.gif' border='0'></a></TD></TR>
					<TR><TD>
						 <TEXTAREA NAME="content" rows=36 cols=88  class='text_01'><%=content%></TEXTAREA></TD></TR>
			   </TBODY></TABLE>			
		   </td></tr>
		 <tr bgcolor="C7C7C7"><td height="1" colspan="4"></td></tr>
		 <tr>
           ...
           <td width="87%" height="25" colspan="3" class="bg_04">
			<% 
			for(int i=0,no=1; i<cnt; i++,no++) {	//기존내용 보여주기
				if(addFile[i][0].length() == 0) {		//내부적으로 삭제된 첨부일때
			...
			cnt++;		//attachefile의 번호를 순서를 위해
			if(cnt < attache_cnt) {							//첨부파일 추가하기
...
function attachDel(a)
{
	...
	document.eForm.action='bogo_FP_Rewrite.jsp';
...
{
	wopen("searchProjectName.jsp?target=document.eForm.prj_name","search_project","500","307","scrollbars=yes,toolbar=no,status=no,resizable=yes");
...
function eleApprovalRequest()
{
	...
	if(decision == 0) { alert("승인자를 입력하십시오."); return; }

	if(!document.eForm.subject.value){
		alert('제목을 입력하십시오.');
		document.eForm.subject.focus();
		return;
	}
	if(!document.eForm.content.value){
		alert('내용을 입력하십시오.');
		document.eForm.content.focus();
		return;
	}
	//제목구하기
	var subject = document.eForm.subject.value;
	var doc_sub = "보고서 : "+subject;

	document.onmousedown=dbclick;
	//일괄합의 결재상신진행
	document.eForm.action='../../../servlet/BogoSeoServlet';
	document.eForm.mode.value='R_BOGO';	
...
function eleApprovalTemp()
{
	...
	document.eForm.action="../../../servlet/BogoSeoServlet";
	document.eForm.mode.value='R_BOGO_TMP';
...
function sel_pjt_code() {
	para = "&target=eForm.pjt_code/eForm.prj_name";	wopen('../../../servlet/PsmProcessServlet?mode=search_project'+para,'search_pjt','400','228','scrollbars=no,toolbar=no,status=no,resizable=no');
}

function FormResizeInc(obj) {
	obj.rows += 3;
}

function FormResizeDec(obj) {
	obj.rows -= 3;
}
