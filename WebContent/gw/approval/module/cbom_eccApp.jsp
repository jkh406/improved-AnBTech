...
<%
	...
	*********************************************************************/	
	...
	String pid = request.getParameter("pid");	if(pid == null) pid = "";			//설계변경 관리번호
	
	/*********************************************************************
	 	설계변경정보의 필요정보 찾기
	*********************************************************************/
	String fname = "";				//첨부파일 명
	String attache_cnt = "0";		//첨부파일 갯수
	String eco_pid = "";			//관리번호
	String eco_no = "";				//설계변경번호
	String subject = "";			//제목
	
	String[] odColumn = {"pid","ecc_subject","eco_no"};
	bean.setTable("ecc_com");			
	...
	while(bean.isAll()) {
		eco_pid = bean.getData("pid");	
		subject = bean.getData("ecc_subject");				
		eco_no = bean.getData("eco_no");					
	} //while

	//ECR 첨부파일 갯수 구하기
	String[] reqColumn = {"pid","fname"};
	bean.setTable("ecc_req");			
	bean.setColumns(reqColumn);
	query = "where pid ='"+eco_pid+"'";
	bean.setSearchWrite(query);
	bean.init_write();
	if(bean.isAll()) fname = bean.getData("fname");

	//ECO첨부파일 갯수 구하기
	String[] ordColumn = {"pid","fname"};
	bean.setTable("ecc_ord");			
	bean.setColumns(ordColumn);
	query = "where pid ='"+eco_pid+"'";
	bean.setSearchWrite(query);
	bean.init_write();
	if(bean.isAll()) fname += bean.getData("fname");
	...
	attache_cnt=Integer.toString(cnt);
	...
	<TR><TD height=27><!--타이틀-->
		<TABLE height=27 cellSpacing=0 cellPadding=0 width="100%">
		  <TBODY>
			...
			<TR bgcolor="#BAC2CD">
			  <TD valign='middle' class="title"><img src="../../images/blet.gif"> 설계변경 결재상신</TD></TR></TBODY>
		</TABLE></TD></TR>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr><td align="center"><iframe id="iframe_main" src="../../../servlet/CbomProcessServlet?mode=ecc_app&pid=<%=pid%>&eco_no=<%=eco_no%>" width="100%" height="640" marginwidth="0" marginheight="0" hspace="0" vspace="0" frameborder="0" scrolling="no">
</iframe></td></tr></table>

<input type='hidden' name='doc_id' value='<%=bean.getID()%>'>
<input type='hidden' name='date' value='<%=anbdt.getTime()%>'>
<input type='hidden' name='doc_sub' value='설계변경 : <%=subject%>'>
...
<input type='hidden' name='mode' value=''>					<% //설계변경 종류 %>
<input type='hidden' name='id' value='<%=pid%>'>			<% //설계변경 관리번호 %>
<input type='hidden' name='eco_no' value='<%=eco_no%>'>		<% //설계변경 번호 %>
...
function eleApprovalRequest()
{
	...
	//일괄합의 결재상신진행
	document.eForm.action='../../../servlet/BomApprovalServlet';	
	document.eForm.app_mode.value='REQ';
	document.eForm.mode.value='ecc_app';
