...
<%
	...
	*********************************************************************/	
	...
	/*********************************************************************
	 	전달변수
		estimate_no : 견적번호, ver:견적버젼
	*********************************************************************/	
	String line = request.getParameter("doc_app_line"); if(line == null) line = "";	//결재선
	String estimate_no = request.getParameter("estimate_no");	
		if(estimate_no == null) estimate_no = "";	//견적번호
	String ver = request.getParameter("ver");			if(ver == null) ver = "";	//견적버젼
	
	/*********************************************************************
	 	견적서에서 제목만들기
	*********************************************************************/	
	String[] tColumn = {"estimate_subj"};
	bean.setTable("estimate_info");			
	bean.setColumns(tColumn);
	bean.setOrder("estimate_subj ASC");	
	query = "where estimate_no ='"+estimate_no+"'";
	bean.setSearchWrite(query);
	bean.init_write();
	if(bean.isAll()) subject = bean.getData("estimate_subj");
%>
	...
	<TR><TD height=27><!--타이틀-->
		<TABLE height=27 cellSpacing=0 cellPadding=0 width="100%">
		  <TBODY>
			...
			<TR bgcolor="#BAC2CD">
			  <TD valign='middle' class="title"><img src="../../images/blet.gif"> 견적서</TD></TR></TBODY>
		</TABLE></TD></TR>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr><td width="100%" height="100%" valign="top">
		<iframe id="iframe_main" src="../../../servlet/EstimateMgrServlet?mode=print&estimate_no=<%=estimate_no%>&ver=<%=ver%>" width="100%" height="600" marginwidth="0" marginheight="0" hspace="0" vspace="0" frameborder="0" scrolling="no"></iframe>
  </td></tr></table>
...
<input type='hidden' name='estimate_no' value='<%=estimate_no%>'>		<% //견적 번호 %>
<input type='hidden' name='ver' value='<%=ver%>'>						<% //견적 버젼 %>
...
function eleApprovalRequest()
{
	...
	//제목구하기
	var doc_sub = "견적서 : "+'<%=subject%>';

	//일괄합의 결재상신진행
	document.eForm.action='../../../servlet/EstimateAppServlet';	
