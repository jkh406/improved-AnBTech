...
<%
	...
	*********************************************************************/	
	...
	//기술문서 관리번호 분리해 찾아내기
	String tablename	="";			//기술문서 table name
	String d_id			= "";			//기술문서 관리번호
	String ver			= "";			//기술문서 version
	String category		= "";			//기술문서 분류코드

	StringTokenizer mod_id = new StringTokenizer(doc_id,"|");
	int m = 0;
	while(mod_id.hasMoreTokens()) {
		if(m == 0) d_id = mod_id.nextToken();
		else if(m == 1) tablename = mod_id.nextToken();
		else if(m == 2) ver = mod_id.nextToken();
		m++;
	}
	...
	<TR><TD height=27><!--타이틀-->
		<TABLE height=27 cellSpacing=0 cellPadding=0 width="100%">
		  <TBODY>
			...
			<TR bgcolor="#BAC2CD">
			  <TD valign='middle' class="title"><img src="../../images/blet.gif"> 기술문서</TD></TR></TBODY>
		</TABLE></TD></TR>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr><td width="100%" height="100%" valign="top">
		<iframe id="iframe_main" src="../../../servlet/AnBDMS?tablename=<%=tablename%>&mode=report&d_id=<%=d_id%>&ver=<%=ver%>" width="100%" height="100%" marginwidth="0" marginheight="0" hspace="0" vspace="0" frameborder="0" scrolling="no"></iframe>
  </td></tr></table>
...
{
	wopen('../../../servlet/ApprovalProcessServlet?PID=<%=pid%>&mode=PRS&link_id=<%=doc_id%>&app_id=<%=pid%>&tablename=<%=tablename%>&ver=<%=ver%>',"eleA_app_decision","400","200","scrollbar=no,toolbar=no,status=no,resizable=no");
}
//반려하기
function winReject()
{
	wopen('../../../servlet/ApprovalProcessServlet?PID=<%=pid%>&mode=PRS_REJ&link_id=<%=doc_id%>&app_id=<%=pid%>&tablename=<%=tablename%>&ver=<%=ver%>',"eleA_app_decision","400","200","scrollbar=no,toolbar=no,status=no,resizable=no");
}
