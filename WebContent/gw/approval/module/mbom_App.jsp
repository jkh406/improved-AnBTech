...
<%
	...
	*********************************************************************/	
	...
	String pid = request.getParameter("pid");	if(pid == null) pid = "";		//BOM기본정보 관리번호
	...
	*********************************************************************/	
	...
	String[] odColumn = {"model_code","model_name","fg_code"};
	...
	while(bean.isAll()) {
		model_code = bean.getData("model_code");				
		model_name = bean.getData("model_name");		
		fg_code = bean.getData("fg_code");				
	} //while
	subject = model_name+"["+model_code+"/"+fg_code+"]에 대한 BOM 작성완료 결재상신";
%>
	...
	<TR><TD height=27><!--타이틀-->
		<TABLE height=27 cellSpacing=0 cellPadding=0 width="100%">
		  <TBODY>
			...
			<TR bgcolor="#BAC2CD">
			  <TD valign='middle' class="title"><img src="../../images/blet.gif"> BOM 결재상신</TD></TR></TBODY>
		</TABLE></TD></TR>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr><td align="center"><iframe id="iframe_main" src="../../../servlet/BomBaseInfoServlet?mode=info_preapp&pid=<%=pid%>" width="100%" height="600" marginwidth="0" marginheight="0" hspace="0" vspace="0" frameborder="0" scrolling="no">
</iframe></td></tr></table>

<input type='hidden' name='doc_id' value='<%=bean.getID()%>'>
<input type='hidden' name='date' value='<%=anbdt.getTime()%>'>
<input type='hidden' name='doc_sub' value='BOM : <%=subject%>'>
...
<input type='hidden' name='mode' value=''>				<% //BOM 종류 %>
<input type='hidden' name='id' value='<%=pid%>'>		<% //BOM 관리번호 %>
...
function eleApprovalRequest()
{
	...
	document.eForm.mode.value='info_app';
