...
<%
	...
	*********************************************************************/	
	...
	String req_no = request.getParameter("request_no");	if(req_no == null) req_no = "";		//구매요청번호
	String req_type = request.getParameter("request_type");if(req_type == null) req_type ="";//구매방법

	/*********************************************************************
	 	구매정보의 필요정보 찾기
	*********************************************************************/	
	String project_code="";			//프로젝트코드
	String project_name="";			//프로젝트명
	String subject = "";			//제목
	
	String[] puColumn = {"project_code","project_name"};
	bean.setTable("pu_requested_info");			
	bean.setColumns(puColumn);
	query = "where request_no ='"+req_no+"'";
	bean.setSearchWrite(query);
	bean.init_write();

	if(bean.isAll()) {
		project_code = bean.getData("project_code");				
		project_name = bean.getData("project_name");	
		subject	= project_name;
		if(project_name.equals("")) subject = project_code;
	} //while
	
    if(subject.equals("")) subject = req_no;

	subject = subject+" 에 대한 구매요청 결재상신";
	...
	<TR><TD height=27><!--타이틀-->
		<TABLE height=27 cellSpacing=0 cellPadding=0 width="100%">
		  <TBODY>
			...
			<TR bgcolor="#BAC2CD">
			  <TD valign='middle' class="title"><img src="../../images/blet.gif"> 구매요청 결재상신</TD></TR></TBODY>
		</TABLE></TD></TR>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr><td align="center"><iframe id="iframe_main" src="../../../servlet/PurchaseMgrServlet?mode=request_app_view&request_no=<%=req_no%>&request_type=<%=req_type%>" width="100%" height="500" marginwidth="0" marginheight="0" hspace="0" vspace="0" frameborder="0" scrolling="no">
</iframe></td></tr></table>

<input type='hidden' name='doc_id' value='<%=bean.getID()%>'>
<input type='hidden' name='date' value='<%=anbdt.getTime()%>'>
<input type='hidden' name='doc_sub' value='구매요청 : <%=subject%>'>
<input type='hidden' name='attache_cnt' value='0'>
<input type='hidden' name='doc_per' value='1'>
<input type='hidden' name='doc_sec' value='GENER'>
<input type='hidden' name='mode' value=''>				
<input type='hidden' name='req_no' value='<%=req_no%>'>			<% //구매요청번호 %>
<input type='hidden' name='req_type' value='<%=req_type%>'>		<% //구매요청TYPE %>
...
function eleApprovalRequest()
{
	...
	//일괄합의 결재상신진행
	document.eForm.action='../../../servlet/purchaseApprovalServlet';	
	document.eForm.app_mode.value='REQ';
	document.eForm.mode.value='purchase_app';
