...
<%
	...
	*********************************************************************/	
	...
	bean.init_write();
	while(bean.isAll()) {
		v_no = bean.getData("car_no");				//차량명
		v_model = bean.getData("model_name");		//모델명
	}

	/*********************************************************************
	 	내부 데이터 처리
	*********************************************************************/	
	line = request.getParameter("doc_app_line"); if(line == null) line = "";	//결재선
%>
...
<!-- 결재 정보 -->
<form action="baecha_sincheong_FP_App.jsp" name="eForm" method="post" encType="multipart/form-data" style="margin:0">
...
<input type='hidden' name='doc_id' value='<%=link_id%>'>
...
function eleApprovalRequest()
{
	...
	document.eForm.mode.value='BAE_CHA';	
