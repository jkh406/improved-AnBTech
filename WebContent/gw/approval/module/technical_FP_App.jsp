...
<%
	...
	*********************************************************************/	
	...
	String tablename = request.getParameter("tablename");if(tablename == null) tablename = "";	//table명
	String data_id = request.getParameter("data_id");	if(data_id == null) data_id = "";//문서 관리번호
	String ver = request.getParameter("ver");	if(ver == null) ver = "";				 //문서 ver
	String category = request.getParameter("category");

	/*********************************************************************
	 	기술문서에서 제목만들기
	*********************************************************************/	
	String[] tColumn = {"subject"};
	//bean.setTable("techdoc_data");
	bean.setTable(tablename);
	bean.setColumns(tColumn);
	bean.setOrder("subject ASC");	
	query = "where ancestor ='"+data_id+"'";
	bean.setSearchWrite(query);
	bean.init_write();
	if(bean.isAll()) subject = bean.getData("subject");	
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr><td width="100%" height="100%" valign="top">
		<iframe id="iframe_main" src="../../../servlet/AnBDMS?tablename=<%=tablename%>&mode=report&d_id=<%=data_id%>&ver=<%=ver%>&category=<%=category%>" width="100%" height="100%" marginwidth="0" marginheight="0" hspace="0" vspace="0" frameborder="0" scrolling="no"></iframe>
  </td></tr></table>
...
<input type='hidden' name='tablename' value='<%=tablename%>'>		<% //table name %>
<input type='hidden' name='mode' value='report'>					<% //기술문서임 %>
<input type='hidden' name='d_id' value='<%=data_id%>'>				<% //기술문서 관리번호 %>
<input type='hidden' name='ver' value='<%=ver%>'>					<% //기술문서 ver %>
...
function eleApprovalRequest()
{
	...
	//제목구하기
	var doc_sub = "기술문서 : "+'<%=subject%>';

	//일괄합의 결재상신진행
	document.eForm.action='../../../servlet/TechnicalServlet';	
