...
<%
	...
	*********************************************************************/	
	...
	/*********************************************************************
	 	전달변수
		승인종류:report_w[신규등록],report_r[사양변경],report_d[삭제요청]
	*********************************************************************/	
	String line = request.getParameter("doc_app_line"); if(line == null) line = "";	//결재선
	String mode = request.getParameter("mode");			if(mode == null) mode = "";	//승인 종류
	String no = request.getParameter("no");				if(no == null) no = "";	//승인 관리번호
	
	/*********************************************************************
	 	승인원에서 제목만들기
	*********************************************************************/	
	String[] tColumn = {"item_desc"};
	bean.setTable("ca_master");			
	bean.setColumns(tColumn);
	bean.setOrder("item_desc ASC");	
	if(mode.equals("report_d")) query = "where mid ='"+no+"'";		//승인취소 결재상신
	else query = "where tmp_approval_no ='"+no+"'";					//신규승인
	bean.setSearchWrite(query);
	bean.init_write();
	if(bean.isAll()) subject = bean.getData("item_desc");	
	...
	<TR><TD height=27><!--타이틀-->
		<TABLE height=27 cellSpacing=0 cellPadding=0 width="100%">
		  <TBODY>
			...
			<TR bgcolor="#BAC2CD">
			  <TD valign='middle' class="title"><img src="../../images/blet.gif"> 승인원</TD></TR></TBODY>
		</TABLE></TD></TR>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr><td width="100%" height="100%" valign="top">
		<iframe id="iframe_main" src="../../../servlet/ComponentApprovalServlet?mode=<%=mode%>&no=<%=no%>" width="100%" height="100%" marginwidth="0" marginheight="0" hspace="0" vspace="0" frameborder="0" scrolling="no"></iframe>
  </td></tr></table>
...
<input type='hidden' name='mode' value='<%=mode%>'>		<% //승인원 종류 %>
<input type='hidden' name='no' value='<%=no%>'>			<% //승인원 관리번호 %>
...
function eleApprovalRequest()
{
	...
	//제목구하기
	var doc_sub = "승인원 : "+'<%=subject%>';

	//일괄합의 결재상신진행
	document.eForm.action='../../../servlet/AcknowledgmentServlet';	
