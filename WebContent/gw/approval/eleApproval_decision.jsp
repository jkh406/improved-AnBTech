	   ...
	   <tbody>
         ...
         <tr>
           ...
           <td width="80%" height="25" class="bg_02">
				...
				<input type="hidden" name="mode" value='PRS'>
...
<% } else if(pass.equals("OK")) { %>
	<table cellspacing=0 cellpadding=2 width="94%" border=0><!-- 비밀번호 확인 -->
	   <tbody>
         <tr><td align="right" height=20 colspan="2"></td></tr>
         <tr bgcolor="c7c7c7"><td height=1 colspan="2"></td></tr>
         <tr>
           <td width="20%" height="25" class="bg_01" background="../gw/images/bg-01.gif">간단의견 <br>(100자)</td>
           <td width="80%" height="25" class="bg_02"><TEXTAREA NAME="app_comment" rows=5 cols=35 maxlength="100" onKeyUp="charCount();" onMouseUp="charCount();"></TEXTAREA>
				<input type="hidden" name="pid" value='<%=pid%>'>
				<input type="hidden" name="link_id" value='<%=link_id%>'>
				<input type="hidden" name="app_id" value='<%=app_id%>'>
		  </td></tr>
...
function elePassword()
{
	...
	var data = "mode="+'PRS'+"&PID="+pid+"&passwd="+passwd;	
...
function eleProcess()
{
	...
	if(StringLength(comment) > 100) {
		alert("승인의견을 100자이내로 입력하십시요.");	
		return;
	} 
	...
	var data = "mode="+'APP'+"&PID="+pid+"&comment="+comment+"&path="+path;		//공통
