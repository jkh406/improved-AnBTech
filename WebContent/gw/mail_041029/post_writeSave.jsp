...
<%
	...
	*********************************************************************/
	...
	} else if(RES.equals("TMP")) {			//미전송[저장] (POST_MASTER만 저장함)
		m_inputs = "INSERT INTO POST_MASTER(pid,post_subj,writer_id,writer_name,write_date,post_receiver,isopen,post_state,post_select,bon_path,bon_file,";
		m_inputs += "add_1_original,add_1_file,add_2_original,add_2_file,add_3_original,add_3_file,delete_date) values('";
		m_inputs += pid + "','" + subject + "','" + id + "','" + name + "','" + up_date + "','" + LIST + "','" + "0" + "','";
		m_inputs += RES + "','" + SEL_DATA + "','" + conDir + "','" + text_file + "','" + File_Name1 + "','" + nFile1 + "','";
	...
	//-----------------------------------------------------------
	//  미전송내용을 다시 전송 또는 미전송시 첨부파일 및 본문 처리
	//-----------------------------------------------------------/
	//본문파일 삭제하기
	String Textfile = upload_path + Bpath + "/" + pfie;
	Rtext.delFilename(Textfile);	   		
		
	//첨부파일1 update
	if(pad1o.length() > 0) { 
		String query1 = "update POST_MASTER set add_1_original = '" + pad1o + "',";
			  query1 += "add_1_file = '" + pad1f + "' where pid='" + pid + "'"; 
		try { bean.execute(query1);  } catch (Exception e) { out.println("임시 첨1up : " +e); } 
	}
	//첨부파일2 update
	if(pad2o.length() > 0) { 
		String query2 = "update POST_MASTER set add_2_original = '" + pad2o + "',";
			  query2 += "add_2_file = '" + pad2f + "' where pid='" + pid + "'"; 
		try { bean.execute(query2); } catch (Exception e) { out.println("임시 첨2up : " +e); } 
	}
	//첨부파일3 update
	if(pad3o.length() > 0) { 
		String query3 = "update POST_MASTER set add_3_original = '" + pad3o + "',";
			  query3 += "add_3_file = '" + pad3f + "' where pid='" + pid + "'"; 
		try { bean.execute(query3); } catch (Exception e) { out.println("임시 첨3up : " + e); } 
	}
%>
