...
function tree (a_items, a_template) {
	...
	this.a_children = [];
	for (var i = 0; i < a_items.length; i++) // root의 갯수만큼 만복
...
function item_init () {
	...
	return '<table cellpadding="0" cellspacing="0" border="0"><tr><td nowrap>' + (this.n_depth ? a_offset.join('') + (this.a_children.length
		...
		+ '<a href="#" onclick="javascript:top.opener.location.href=\'../servlet/' + this.a_config[1] + '&o_root=' + this.o_root.n_id + '&n_id=' + this.n_id + '&n_depth=' + this.n_depth + '&mode=view_model\';return trees[' + this.o_root.n_id + '].select(' + this.n_id + ');" ondblclick="trees[' + this.o_root.n_id + '].toggle(' + this.n_id + ')" onmouseover="trees[' + this.o_root.n_id + '].mover(' + this.n_id + ')" onmouseout="trees[' + this.o_root.n_id + '].mout(' + this.n_id + ')" class="t' + this.o_root.n_id + 'i" id="i_txt' + this.o_root.n_id + '_' + this.n_id + '"><img src="' + this.get_icon() + '" border="0" align="absbottom" name="i_img' + this.o_root.n_id + '_' + this.n_id + '" class="t' + this.o_root.n_id + 'im">' + this.a_config[0] + '</a></td></tr></table>' + (this.a_children.length ? '<div id="i_div' + this.o_root.n_id + '_' + this.n_id + '" style="display:none"></div>' : '');
}
