...
<%
	...
	if(searchword != null){
		query = "SELECT COUNT(*) FROM goods_structure WHERE name LIKE '%" + searchword +"%'";
		
		bean.executeQuery(query);
		if(bean.next()) result_count = Integer.parseInt(bean.getData(1));

		if(result_count == 1){
			query = "SELECT pid FROM goods_structure WHERE name LIKE '%" + searchword +"%'";
...
%>
		<script language='javascript'>
			window.open("multi_result_view.jsp?target=GoodsTreeForModelSearch.jsp&searchword=<%=searchword%>&searchscope=<%=searchscope%>",'search','width=600,height=100,scrollbars=yes,toolbar=no,status=no,resizable=no');
		</script>
<%
		}
	}
...
<TABLE height="100%" cellSpacing=0 cellPadding=0 width=100% border=0>
  ...
  <TR>
    <TD style="PADDING-LEFT: 15px; PADDING-TOP: 10px" vAlign=top>
		...
		<script language="JavaScript" src="admin/goodsTreeItemsForModelSearch.js"></script>
		<script language="JavaScript" src="admin/treeForModelSearch.js"></script>
