...
<%
	...
	if(table_iter.hasNext()){
		...
		module_add=table.getModuleAdd();			//보낼주소
		mail=table.getMail();						//전자우편종류
		mail_add=table.getMailAdd();				//전자우편주소(사번/이름;)
	}
	//수신인 부서명만 출력하기
	String rec_div = "";
	StringTokenizer rdata = new StringTokenizer(receive,";");		//파일명 담기
	while(rdata.hasMoreTokens()) {
		String rec = rdata.nextToken();
		if(rec.length() > 2) {
			int slen = rec.indexOf("/")+1; if(slen == -1) slen = 0;
			rec_div += rec.substring(slen,rec.length())+"장,";
		}
	}
	receive = rec_div.substring(0,rec_div.length()-1);
