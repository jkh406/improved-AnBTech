...
function goPage(a) 
{
	document.sForm.action='../servlet/OutDocumentRecServlet';
...
function write()
{
	document.sForm.action='../ods/OutDocumentRec_write.jsp';
	document.sForm.submit();
}
...
{
	document.sForm.action='../servlet/OutDocumentRecServlet';
	document.sForm.mode.value='ODR_M';
...
function contentDelete(id)
{
	...
	document.dForm.action='../servlet/OutDocumentRecServlet';
	document.dForm.mode.value="ODR_D";
...
//배포하기
function contentDistribute(id,subj)
{
	var mode = "ODR_G";
	var tablename = "OutDocument_receive";
	//window.open("../ods/shareReceiver.jsp?target=sForm&id="+id+"&tablename="+tablename+"&subj="+subj+"&mode="+mode,"proxy","width=510,height=467,scrollbar=yes,toolbar=no,status=no,resizable=no");
...
{
	wopen('OutDocumentRecServlet?mode=ODR_V&id='+id,'odr','510','322','scrollbars=yes,toolbar=no,status=no,resizable=no');
}
    ...
    <TD vAlign=top>
		<TABLE height=27 cellSpacing=0 cellPadding=0 width="100%">
		  <TBODY>
			...
			<TR bgcolor="#BAC2CD">
			  <TD valign='middle' class="title"><img src="../ods/images/blet.gif" align="absmiddle"> 사외공문수신</TD>
		...
		<TABLE height=32 cellSpacing=0 cellPadding=0 width="100%" border='0'>
			...
			<TR>
			  ...
			  <TD align=left width='400'>
					...
					<%
						String[] sitems = {"subject","receive","sending"};
						String[] snames = {"제목","수신","발신"};
						String sel = "";
				...
				<%  if(idx != -1) { //사외작성 권한에 대한 등록메뉴 출력여부 판단하기	%>
						<a href='javascript:write()'><img src="../ods/images/bt_add_new2.gif" border='0' align='absmiddle'></a>
				<%	}	%>

					<input type="hidden" name="mode" size="15" value="ODR_L">
  ...
  <TR height=100%>
	  ...
	  <TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
        <TBODY>
			<TR vAlign=middle height=25>
			  ...
			  <TD noWrap width=6 class='list_title'><IMG src="../ods/images/list_tep.gif"></TD>
			  <TD noWrap width=80 align=middle class='list_title'>작성일자</TD>
			  <TD noWrap width=6 class='list_title'><IMG src="../ods/images/list_tep.gif"></TD>
			  <TD noWrap width=100 align=middle class='list_title'>수신처</TD>
			  <TD noWrap width=6 class='list_title'><IMG src="../ods/images/list_tep.gif"></TD>
	...
	<% 
		while(table_iter.hasNext()) {
			...
			String rec = table.getReceive();		//수신부서
			String rec_name = "";
			if(rec.indexOf("/") != -1) {
				StringTokenizer rdata = new StringTokenizer(rec,";");
				while(rdata.hasMoreTokens()) {
					String rsn = rdata.nextToken();
					if(rsn.length() > 3) {
						int sno = rsn.indexOf("/");
						rec_name += rsn.substring(sno+1,rsn.length())+",";
					}
				}
				rec_name = rec_name.substring(0,rec_name.length()-1);
			} else rec_name = rec;	
	%>	
			<TR onmouseover="this.style.backgroundColor='#F5F5F5'" onmouseout="this.style.backgroundColor=''" bgColor=#ffffff>
			  ...
			  <TD><IMG height=1 width=1></TD>
			  <TD align=middle class='list_bg'><%=rec_name%></TD>
			  <TD><IMG height=1 width=1></TD>
			  <TD align=middle class='list_bg'><%=table.getModify()%>&nbsp;&nbsp;<%=table.getDelete()%>&nbsp;&nbsp;<%=table.getDistribute()%></TD></TR>
