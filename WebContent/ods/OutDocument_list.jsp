...
function goPage(a) 
{
	document.sForm.action='../servlet/OutDocumentServlet';
...
function write()
{
	document.sForm.action='../ods/OutDocument_write.jsp';
	document.sForm.submit();
}
...
function app()
{
	//sParam = "strSrc=OutDocument_App.jsp&id=";
	sParam = "../gw/approval/module/OutDocument_App.jsp?id=";
	...
	if(rval == "RL") {
		...
		document.sForm.action='../servlet/OutDocumentServlet';
		document.sForm.mode.value='OTD_L';
		document.sForm.submit();
	}
	*/
...
{
	//sParam = "strSrc=OutDocumentServlet&mode=OTD_V&id="+id;
	...
	wopen('OutDocumentServlet?mode=OTD_V&id='+id,'view_doc','680','650','scrollbars=yes,toolbar=no,status=no,resizable=no');
}

//개별내용 보기 (결재승인후)
function contentAppview(id,app_id)
{
	//sParam = "strSrc=OutDocumentServlet&mode=OTD_A&id="+id+"&doc_id="+app_id;
	
	//var rval = showModalDialog("../ods/DocModalFrm.jsp?"+sParam,"","dialogWidth:720px;dialogHeight:750px;toolbar=0;location=0;directories=0;status=0;menuBar=0;scrollBars=0;resizable=0");

	wopen('OutDocumentServlet?mode=OTD_A&id='+id+'&doc_id='+app_id,'view_doc','680','650','scrollbars=yes,toolbar=no,status=no,resizable=no');
}
...
{
	document.sForm.action='../servlet/OutDocumentServlet';
	document.sForm.mode.value='OTD_M';
...
function contentDelete(id)
{
	...
	document.dForm.action='../servlet/OutDocumentServlet';
	document.dForm.mode.value="OTD_D";
...
{
	wopen("OutDocumentServlet?mode=ODS_R&id="+id,"ods",'680','650','scrollbars=yes,toolbar=no,status=no,resizable=no');
}
    ...
    <TD vAlign=top>
		<TABLE height=27 cellSpacing=0 cellPadding=0 width="100%">
		  <TBODY>
			...
			<TR bgcolor="#BAC2CD">
			  <TD valign='middle' class="title"><img src="../ods/images/blet.gif" align="absmiddle"> 사외공문</TD>
		...
		<TABLE height=32 cellSpacing=0 cellPadding=0 width="100%" border='0'>
			...
			<TR>
			  ...
			  <TD align=left width='400'>
					...
					<%
						String[] sitems = {"subject","receive","in_date"};
						String[] snames = {"제목","수신","작성일"};
						String sel = "";
	...
	%>	
			<TR onmouseover="this.style.backgroundColor='#F5F5F5'" onmouseout="this.style.backgroundColor=''" bgColor=#ffffff>
			  <TD align=middle height="24" class='list_bg'>
				<% if ((flag == null) || flag.equals("EN") || flag.equals("EC"))	{	//편집가능,미결재상신,반려,상신취소 상태임 %>
