...
<%
	...
	if(app_iter.hasNext()){
		...
		line = "기안 "+wid+" "+wname+" "+app.getGianRank()+" ";
		line += app.getGianDiv()+" "+wdate+"<br>";
		line_cnt++;
		if(vdate.length() > 10) {
			line += "검토 "+vid+" "+vname+" "+app.getReviewRank()+" ";
			line += app.getReviewDiv()+" "+vdate+" "+app.getReviewComment()+"<br>";
			line_cnt++;
		}
		if(adates.length() > 10) {
			int cnt = 0;
			...
			//line만들기
			for(int i=0; i<cnt; i++) {
				line += "협조 "+agree[i][0]+" "+agree[i][1]+" "+agree[i][2]+" "+agree[i][3]+" ";
				line += agree[i][4]+" "+agree[i][5]+"<br>";
				line_cnt++;
			}

		}
		line += "승인 "+did+" "+dname+" "+app.getDecisionRank()+" ";
		line += app.getDecisionDiv()+" "+ddate+" "+app.getDecisionComment()+"<br>";
		line_cnt++;
	}
...
<table width='640' border="0" cellspacing="0" cellpadding="0">
	...
	<td width=50% align='right' height=20>
		<div id="print" style="position:relative;visibility:visible;">
			<a href='Javascript:sendprint();'><img src='../ods/images/bt_out_print.gif' align='absmiddle' border='0' alt='외부발송용 출력'></a> <!-- 외부발송용 출력 -->
...
<TABLE cellSpacing=0 cellPadding=0 width="640" border=1 bordercolordark="white" bordercolorlight="#9CA9BA"> 
	<tr>   
		...
		<% } else {				 %>
			...
			<TEXTAREA rows=7 cols=66 readOnly style="text-align:left;font-size:9pt;border:1px solid #787878;"><%=line%></TEXTAREA>
			</td>
		<% }	%>
...
function sendprint()
{
	wopen('OutDocumentServlet?mode=OTD_S&id=<%=id%>','view_doc','680','650','scrollbars=yes,toolbar=no,status=no,resizable=no');
}
