...
%>
					...
					<% 
						...
						if ("BOM등록".equals(table.getBomStatus()))	{ 
							out.println("<input type='checkbox' name='checkbox' value='"+table.getPid()+"|"+table.getPurpose()+"'>");
						 } 
					%>
						</TD>
...
<input type="hidden" name="bom_status" size="15" value="">
</form>
...
function app()
{
    ...
    for(i=0;i<f.length;i++){
		if(f[i].checked){
			var ck_pid = f[i].value;
			var pid = ck_pid.split("|");
			sParam += pid[0]; 
			s_count++;
		}
    }
	...
	//임시BOM상신
		...
		if(!confirm('임시BOM은 전자결재없이 상신됩니다. 계속하시겠습니까?')) {
			return;
		}
		document.sForm.action='../servlet/BomBaseInfoServlet';
		document.sForm.mode.value='tbom_sangsin';
		document.sForm.pid.value=pid[0];
		document.sForm.bom_status.value='4';
		document.sForm.submit();
	}
}
